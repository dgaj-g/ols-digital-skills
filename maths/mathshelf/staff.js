...
(function () {
  ...
  function showClasses() {
    ...
    function runTickSync(c) {
      ...
      sync.queued = null;
      sync.busy = true;
      var showBoxes = function (state) {
        Object.keys(acts).forEach(function (id) {
          var box = tw.querySelector('input[data-act-id="' + id + '"]');
          if (box) box.checked = !!(state && state[id]);
        });
      };
      var settle = function () { sync.busy = false; if (sync.queued) runTickSync(c); };
      var outcome = function () { clearBusy(cmsg, TT(pending.on ? 'tickOn' : 'tickOff', { book: pending.title, 'class': c.name })); };
      call('setActs', { className: c.name, acts: acts }).then(function (r) {
        ...
        /* THE STORE OFTEN SAVES AND THEN LOSES THE ANSWER ON THE WAY BACK
           (12 Sept 2026, 16:28: his untick's doPost Completed in 5.7 s and the
           front door's apiCall came back 36 s later as relay-failed). A box
           put back on screen after that showed him the opposite of what his
           pupils see. So a relay failure is never answered by guessing: the
           store is asked what it holds, and the boxes show THAT - the outcome
           line if the change landed, the slow-store sentence if it did not. */
        if (r && r.error === 'relay-failed') {
          call('classes').then(function (r2) {
            var mine = (r2 && r2.ok ? (r2.classes || []) : []).filter(function (x) { return x.name === c.name; })[0];
            if (mine) {
              c.acts = mine.acts || {};
              showBoxes(c.acts);
              SURF('set-up', 'tickboxes');
              if (!!c.acts[pending.id] === !!pending.on) outcome();
              else clearBusy(cmsg, SAYS('relay-failed', TT('couldNotSave')));
            } else {
              showBoxes(c.acts);
              clearBusy(cmsg, SAYS('relay-failed', TT('couldNotSave')));
            }
            settle();
          }).catch(function () {
            showBoxes(c.acts);
        ...
        /* any other refusal: the row goes back to the last state the server
           confirmed, and the server's own word is shown */
        showBoxes(c.acts);
