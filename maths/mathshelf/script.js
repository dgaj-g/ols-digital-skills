...
(function () {
  ...
  var MARK_CLASSES = ('.mark-tick,.mark-cross,.wl-mark,.mk-tally,.mk-comment,.staff-tally,' +
    ...
    '.tally-hand,.err-box,.jq-tally,.jq-feedback,.mk-line,.pile-line,.pile-count,.slip-count,.wl-sub,.col-dx-slip,.chip-ticks,.attempt-note,.amber-note,.mark-tick,.mk-amber,.flash-ok');
  ...
  /* THE CELEBRATION IS NOT A MARK. The gold box drawn round a right final
     answer, and the gold star on a finished book, are the two places gold is
     allowed to mean something — and what it means is "well done", not a
     verdict. They declare themselves so the colour law can tell them from a
     mark wearing the wrong colour. */
  var CELEBRATE_CLASSES = '.box-draw,.book-star,.tally-star,.celebrate';
  function stampCelebrations(root) {
    try {
      (root || document).querySelectorAll(CELEBRATE_CLASSES).forEach(function (el) {
        if (!el.hasAttribute('data-celebrate')) el.setAttribute('data-celebrate', '');
      });
    } catch (e) {}
  }
  var stampMarksOnly = stampMarks;
  stampMarks = function (root) { stampMarksOnly(root); stampCelebrations(root); };
  ...
  function openActivity(a) {
    ...
    /* THE SAME COLOUR CANNOT BE BOTH GROUND AND SMALL PRINT. --act-accent is a
       GROUND: cream sits on it and reads at 5.9:1 or better. Used as INK for an
  ...
  function renderContents() {
    ...
    /* WHICH STATE THE CONTENTS PAGE IS IN. It was stamped "fresh" once, in
       index.html, and never again - so "mid-book" and "finished" were declared
       in the registry and rendered by nothing, and a walk that opened the sixth
