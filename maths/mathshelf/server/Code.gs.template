 ...
 * ONE FILE, TWO PROJECTS (MATHS_V4_DESIGN section 7; the split of 12 Sept 2026)
 *
 *   This same Code.gs is pasted into TWO Apps Script projects. The FRONT DOOR
 *   project is bound to the Sheet (it never opens it - doGet runs as the pupil)
 *   and its manifest rests at USER_ACCESSING + DOMAIN for good. The DATA
 *   project is STANDALONE: it opens the Sheet BY ID from the `sheetId` script
 *   property (ss_()), and its manifest rests at USER_DEPLOYING + ANYONE for
 *   good. One project used to carry both deployments and therefore one
 *   manifest, so every DATA cut meant flipping executeAs/access by hand and
 *   flipping it back; two projects means no manifest is ever touched again
 *   (server/DEPLOY.md, "TWO PROJECTS"). The act whitelist comes from the
 *   Config row `acts` (acts_()), so a new book is a client change plus one
 *   Sheet cell - not a server cut.
 *
 ...
 *
 * WHERE THE FILES GO (server/DEPLOY.md, "TWO PROJECTS"):
 *   * this Code.gs into BOTH projects - the bound front-door project and the
 *     standalone DATA project (script properties sheetId + relaySecret)
 *   * an HTML file named exactly "Index" (paste Index.html into it) into the
 *     front-door project only
 * Run initJotter() once from the DATA project, set your staffPasscode.
 *
...
/* EVERY BOOK THE SERVER WILL ACCEPT = the BUILT-IN floor below, UNIONED with
   the Config row `acts` (acts_()). The server refuses any act id that is not
   in the live list ('bad-act'), so a book cannot save a mark, appear on the
   Working Wall or be drilled into until its id is known - qa-store-scale found
   that on 8 September 2026, the day Handling Data was added, when the list was
   this array alone and every new book was a server cut. Since Book A's cut
   (12 Sept 2026) a book is CLIENT-ONLY PLUS ONE CONFIG ROW: in the Config tab,
   Key `acts`, Value a JSON array of ids, e.g.
       ["angles","algebra","stats-quartiles","stats-collect"]
   The built-ins are always in the list, so nothing live can be switched off by
   a bad row, and the app works before the row exists. Everything below derives
   from acts_(), never from this array, and a book arrives UNTICKED everywhere
   because an absent key reads false (rule 17). acts_() READS THE SHEET, so it
   runs only on the DATA side (doPost -> apiRelay -> api*), never in doGet,
   apiCall or the token minting - qa-two-homes executes doGet as a pupil who
   cannot open the Sheet to prove it. */
var ACTS = ['angles', 'algebra', 'stats-quartiles'];
/* an act id is a Sheet key, a class-registry key and a client tile name: lower
   case, digits and hyphens, 2-41 characters, letter first */
var ACT_ID_RE = /^[a-z][a-z0-9-]{1,40}$/;
/* the live list, read once per execution (apiRelay clears it on the way in and
   out; a fresh Apps Script execution starts with it empty anyway) */
var ACTS_LIVE = null;
function acts_() {
  if (ACTS_LIVE) return ACTS_LIVE;
  var out = [], seen = {}, i;
  for (i = 0; i < ACTS.length; i++) { if (!seen[ACTS[i]]) { seen[ACTS[i]] = true; out.push(ACTS[i]); } }
  var row = getConfig_('acts');
  var extra = parseJson_(row);
...
/* THE SHEET, BY ID. The DATA project is standalone since Book A's cut (12 Sept
   2026): it has no active spreadsheet, so it opens the Sheet named by the
...
function ready_() { var ss = ss_(); return !!(ss.getSheetByName(DATA_TAB) && ss.getSheetByName(CONFIG_TAB)); }
function nowIso_() { return new Date().toISOString(); }
function actOk_(a) { return acts_().indexOf(String(a || '')) > -1; }
/* DERIVED FROM acts_(), never re-typed. The first version named angles and
   algebra literally, so a third book added to the list could be ticked in the
   markbook and silently dropped on the way to the Sheet -- the book could never
   be switched on at all. A new book arrives UNTICKED everywhere (an absent key
   reads false), which is the rule; being un-tickable is a bug. The local ACTS
   here is the LIVE list (built-ins plus the Config row), shadowing the built-in
   floor above so the loop reads as it always has. */
