...
## 13 September 2026, 19:08 — FRONT DOOR Version 40 (THE GIST CUT) — a CLIENT-ONLY cut; DATA stays Version 2

From commit `2ae5d45`. `Index.html` md5 `02bf91923db5f5c857f8c04d740f0313` (1,555,436 chars, rolling hash 4247115325) fetched into the editor from the pushed commit on raw.githubusercontent (`cache: no-store`); the first click on the Files list landed on Code.gs and the model check refused the paste; Index.html selected by its list entry, the active model verified as `file_2.html`, the length and rolling hash read back equal to the repo's, saved ("Saved to Drive" after a click into the editor and Cmd+S). `Code.gs` NOT pasted — md5 `21070f13916e9cd00bf87373cd6fd3ac`, byte-identical to Version 37's. Manifest: read at 18:4x for Version 39 (`USER_ACCESSING` + `DOMAIN`) and not opened since — nothing in this cut touches it. Manage deployments → the one active deployment by ID `AKfycbzUZ3bDjcFas_zQ02VrJQCEkPQgEjs3Re4JZ1OQtLACa090AC1B0Md2yUkL4aX81LwP` → edit → "New version" (set by pressing the option in the page, read back as "New version" before Deploy) → **Version 40 on 13 Sept 2026, 19:08**. Same `/exec`. Proof row in the Executions log: `doGet` 19:08:59 (2.242 s, Completed), the door opened for the check.

**What the cut is.** The whole-tree walk of 17:49 (tools/qa/out/steward/full-tree-2.log) stood `sit-teacher` on a Handling Data book at 768 for the first time — the walk now prefers a third book on the class page — and the exercise grid's column gist ("Put the cycle in order", `.grid th .gist`) measured 4.2:1 in rendered pixels against the panel, under the 4.5:1 floor: the maths face at 13.5px and weight 400 in `--pencil` paints paler than its declared colour. `shell.css`: the gist is `--ink` at weight 500. The readability law that caught it is the existing one (control `invisible-class-names` plants its fault); no new control. The 768 walk after the change: GREEN, 55 checks (tools/qa/out/steward/sit-teacher-768-gist.log).

## 13 September 2026, 18:43 — FRONT DOOR Version 39 (THE STAFF-DOOR CUT) — a CLIENT-ONLY cut; DATA stays Version 2

From commit `f364310`. `Index.html` md5 `7b4746048176cfe6e9f8a3fec24a417b` (1,555,122 chars, rolling hash 3200406563) fetched into the editor from the pushed commit on raw.githubusercontent (`cache: no-store`); the first click on the Files list landed on Code.gs again and the model check refused the paste; Index.html selected by its list entry, the active model verified as `file_2.html`, the length and rolling hash read back equal to the repo's, saved (Cmd+S after a click into the editor, the cloud mark settled). `Code.gs` NOT pasted — md5 `21070f13916e9cd00bf87373cd6fd3ac`, byte-identical to Version 37's. Manifest read in the editor before the cut: `USER_ACCESSING` + `DOMAIN`, untouched. Manage deployments → the one active deployment by ID `AKfycbzUZ3bDjcFas_zQ02VrJQCEkPQgEjs3Re4JZ1OQtLACa090AC1B0Md2yUkL4aX81LwP` → edit → "New version" (the Version list took a coordinate click as the row below it twice — Version 37, then 36 — and was set by pressing the "New version" option in the page instead, read back as "New version" before Deploy) → **Version 39 on 13 Sept 2026, 18:43**. Same `/exec`. The live door read straight after on the deployer's account: the passcode box, no Staff button, no "Not you? Switch account" link. Proof rows in the front door's Executions log, both labelled Version 39: `doGet` 18:43:50 (2.111 s, Completed) — the door opened for the check — and `doGet` 18:46:04 (2.278 s, Completed).

**What the cut is.** `shell.css` gave `.btn-quiet` a `display`, and a class rule beats the UA's `[hidden]` rule, so `staffCover()`'s `hidden = true` on the Staff button did nothing: the teacher's own door (the staff link, no class in the address) showed a "Staff" button under the passcode box — a door to the room she was standing in — and the "Not you? Switch account" link, which the staff door never wires (its handler belongs to the pupil cover). `.btn-quiet[hidden] { display: none }` restores it, the same way `.ink-control[hidden]` was restored. Found by the tutorial film's chapter 1 on his ruling of 13 Sept (evening): the film had pressed that button. Law: `sit-teacher` stands on the staff link's door first, at every width — the cover must draw `staff` with the passcode box shown and no Staff button rendered. Control `staff-door-stray-staff-button` (plant `fixture-staff-door-stray-button`: the restore rule taken away) — battery running at the deploy, pasted into MATHS_GATES_AUDIT.md when it ends. The same commit hardens the book-view name law: it fails when it cannot read its evidence, and stands a second time on the switched book, which the walk now chooses as a Handling Data book — the 18:02 battery had switched to Algebra, where the v3 guess is right, and passed the planted fault.
...
**00:3x — the OLD DATA deployment on the bound project (`AKfycbyO6pQnLHujpost5Otxe9oJB2iFdbno3Kxxw5RU51A9prKiqDMrIm__UWuLEDn2f4wo`, Version 32) ARCHIVED**, selected by its Deployment ID and confirmed by the dialog's own name ("Store cut - DATA … (913e0f8)"); nothing had pointed at it since `dataUrl` moved, and a page loaded before the move falls back to the relay, which reaches the new store.
...
| 2026-09-13 18:43 | FRONT DOOR | Version 39 | `USER_ACCESSING` (and `DOMAIN`), read in the editor's manifest before the cut, untouched — THE STAFF-DOOR CUT, client-only: Index.html only, Code.gs unchanged from Version 37 (the staff door hides its Staff button and the switch link; a class rule's display had beaten [hidden]; found by the tutorial film's chapter 1) | f364310 | 7b4746048176cfe6e9f8a3fec24a417b | 21070f13916e9cd00bf87373cd6fd3ac |
| 2026-09-13 19:08 | FRONT DOOR | Version 40 | `USER_ACCESSING` (and `DOMAIN`), read in the editor's manifest at Version 39 and not opened since — THE GIST CUT, client-only: Index.html only, Code.gs unchanged from Version 37 (the exercise grid's column gist is ink at weight 500; it measured 4.2:1 in rendered pixels at 768 on a Handling Data book) | 2ae5d45 | 02bf91923db5f5c857f8c04d740f0313 | 21070f13916e9cd00bf87373cd6fd3ac |
## Proof rows
