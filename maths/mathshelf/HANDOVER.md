...
**Live since 12 September 2026 19:27, from commit `85a5660` (the live artefacts were built at `85a5660`) — THE STORE AND THE FILMS CUT (front door only; `Code.gs` byte-identical, the manifest read before and after and not touched). Ruling 50: every op a Book C film names is now DRAWN — tables filled a cell at a time, a cumulative frequency chart with its points, its curve, its sliding rule and its drops (Exercise 4's three read 9, 6 and 13, the median and quartiles its captions say), number lines with markers and the box plot they make, and stamps that are no longer held invisible by their own entrance. Ruling 51, her side: the outbox re-sends by itself every 20 s once a save passes 30 s and the card clears itself when the store answers, while a refused save is named at once. Ruling 51, the hop itself: MEASURED, not fixed — `Code.gs` is unchanged and the change to make is written out in `PROGRESS.md` as the next session's first step. Three teacher sentences stopped naming a person. On top of the three HOTFIXES of 12 Sept and POLISH CUT 2.**

| | /exec | version | executeAs | who can reach it |
|---|---|---|---|---|
| FRONT DOOR (everybody) | `https://script.google.com/a/macros/c2ken.net/s/AKfycbzUZ3bDjcFas_zQ02VrJQCEkPQgEjs3Re4JZ1OQtLACa090AC1B0Md2yUkL4aX81LwP/exec` | Version 31, 12 Sept 19:27 | USER_ACCESSING | anyone in `c2ken.net` |
