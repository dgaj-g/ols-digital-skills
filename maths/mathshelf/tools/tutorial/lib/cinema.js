...
const GOLD = '#E4B824';
/* MathShelf's copy of the OLS cinema layer (13 Sept 2026): the same overlay, laws
   and house style as the KS3 DT tutorial films. The display font is read from
   the KS3 platform's vendored copy — read-only, never written to. */
const FONT_B64 = fs.readFileSync(
  '/Users/damiengartland/Sites/ols-digital-skills/ks3-dt/platform/assets/fonts/space-grotesk.woff2'
).toString('base64');
...
function pageRuntime() {
  ...
  C.curtain = function (spec) {
    ...
    if (!spec.bare) {
      ...
      brand.textContent = spec.brand || 'OUR LADY\u2019S GRAMMAR SCHOOL \u00b7 NEWRY';   /* MathShelf's cards carry the school, not the DT platform */
...
const LAW_MODE = (process.env.MS_FILM_LAWS || process.env.KS3DT_FILM_LAWS || 'enforce').toLowerCase();
