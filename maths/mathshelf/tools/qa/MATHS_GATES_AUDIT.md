...
| 51 (the hop itself) | the front door's UrlFetchApp round trip to the DATA web app is 4.9–37.8 s for a call whose own execution is 0.8–2.0 s, and the same request from outside Google is 2.9–3.4 s. MEASURED 12 Sept 2026 (`timingCheck`, three rounds, PROGRESS.md): the email lookup is 1 ms and the redirect 0.2–0.5 s, so neither is the cost. Nothing in Code.gs shortens the hop. | next session, first step: drop the bearer (B2 — it caused a 404 in two of three probes and a 404 is what becomes `relay-failed`), then re-measure; after that, fewer hops per screen rather than a faster hop, as a cut of its own |
| (none open) | | |
