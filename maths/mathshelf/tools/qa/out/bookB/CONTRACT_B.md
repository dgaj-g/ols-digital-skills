...
## Films (§19 B·s1–s5) use existing ops only: write, ring, table, tcell, stamp, box, note, tick. CORRECTION (13 Sept, 01:30): player.js has NO `bracket` op — the first draft of this line was wrong; `ring {i}` rings the i-th value of the last written line (paperRing), and an odd-count median is one ringed value. A "circle the middle two" beat would need a new op and is not in Book B.
