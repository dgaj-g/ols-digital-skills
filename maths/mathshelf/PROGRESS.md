...
**PROOF of the book-view cut (filled as it lands):** `run.js --fast` — pending · `run.js --full --book stats-averages` (sit-teacher at 375/768/1280 with the new law) — pending · the teacher transcript re-extracted and the TEACHER verdict re-filed against it (the one changed sentence is the header, now naming the book) — pending · FRONT DOOR Version 38 — pending · `control.js --only sit-teacher` for `bookview-wrong-book-name` — runs in the background after the deploy (the audit says so until its FIRED line is pasted).
