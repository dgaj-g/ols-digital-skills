...
| F38 | **the pupil's name was never read.** `doGet` only called `autoName_()` when `getEffectiveUser()` matched `getActiveUser()` - a guard written to stop the DATA deployment printing the deployer's name on a pupil's cover, for a deployment that never serves a page. When `getEffectiveUser()` comes back empty in the deployed context the guard silently killed the lookup and dropped every pupil into "write it once" | him: "I shouldn't have to type my name in - I don't have to do it in the KS3 DT platform" | the guard is gone; the KS3 DT platform, which has always filled the name in, never had one |
