...
**PROOF (filled as it lands):** `run.js --fast` — pending · `control.js --only sit-teacher` — tickbox-same-name FIRED (8 fails), row-control-wraps FIRED (8), ledger-pushes-the-page re-run pending, over-tightening PASSES (94/0) · `control.js --only qa-waits` — pending · `run.js --full --book stats-averages` — pending · the cold read re-filed (TEACHER and stats-averages, against the new transcript hashes) — pending · qa-audit green.

**SERVER SIDE, FOR OPUS (a Code.gs change, both projects):** `Maths/MATHS_STORE_SERVER_BOUNCE_OPUS_PROMPT.txt` — the relay's UrlFetch re-sends once on the bounce and logs the code; the DATA project's doGet answers JSON; the whole-tab scan under the lock is TIMED before anything is redesigned.
