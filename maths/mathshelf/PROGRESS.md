...
## 13 SEPT 2026, 14:20 → — THE TUTORIAL FILMS, and the book-view name cut they found (Fable steward; his order at 14:1x: "create a video tutorial for the Maths department … captions and no audio … the whole mechanics … then a pupil's side as well … do it all in the same session")

**TWO SILENT CAPTIONED FILMS, delivered to `Claude Work/Maths/Tutorials/`:** `MathShelf - A Guide for Teachers.mp4` (26:15, 45 MB, eleven chapters: the door, Set-up, the six pupil chapters, the markbook) and `MathShelf - How to use it (pupils).mp4` (17:24, 32 MB, the six pupil chapters numbered from 1 with their own cards), each with a `chapters-*.json`, the script of every sentence, the separated reader's verdicts and a README. Built in `tools/tutorial/` on the same cinema layer as the KS3 DT films (a read-only copy of `lib/cinema.js`, the display font read from the KS3 platform): `scenes/chapters.js` is the one home of every caption; `record.js` records one browser context for the whole film (so the class the teacher ticks in chapter 2 is the shelf the pupil sees in chapter 3, and the pupil's answers are what the markbook shows in chapter 9) with one page per scene; `assemble.js` trims to the lift/down marks and concatenates. Filmed on the offline preview (localhost:8099): the practice teacher and the practice class of twelve made-up pupils; a made-up pupil "Ciara Walsh" named through the app's own setname; the preview's banner removed and its practice address shown as the practice teacher's; copied links and the QR carry the REAL front-door address (`GJ.app.boot.baseUrl` set on each page); saves held for the seconds a live save takes so the "Saving…" line can be seen (`OLS_TRANSPORT` shim in `lib/film.js`, saves only).

**THE LAWS HE SET FOR THESE FILMS, KEPT:** a ring is drawn only after its target is scrolled into the frame and the take refuses otherwise (five takes died that way today and were re-authored, never forced: a whole table, a whole grid, a strip at the top edge, an ink control of zero size); every scroll is on camera (smooth `scrollIntoView`, then the wait); the pointer parked clear of every text box; a caption never over its own subject; the school's machines' Windows identity; the reading-time hold per word. **A separated cold read of all 110 lines before recording** (`MATHSHELF_TUTORIAL_COLD_READ.md`: 83 PASS / 10 REWRITE / 16 FAIL — verbless fragments, an undefined term, "she" for a generic pupil in the teacher register, a rhetorical triple on a card; every one honoured; two adjusted to stay true to the screen: the grouped table's own headings say "class", and the button is called "Download CSV" on screen so the caption says what it actually does). **Every chapter's finished frames checked against its captions** (contact sheets in `tools/tutorial/out/audit/`).

**WHAT THE FILM FOUND IN THE APP:** (1) **the pupil's book in the markbook named every book but Angles "Algebra"** — staff.js's book-view header kept a v3 two-book guess (`view.act === 'angles' ? 'Angles' : 'Algebra'`) — an Averages book headed "Algebra" on camera at 22:5x of the first take. Fixed in this cut: the header names `bookTitle(view.act)`; sit-teacher law (book-view:pencil at every width: the header carries the book the trail named) with control `bookview-wrong-book-name` (plant: the old guess). (2) The add-a-class line "added with both books on its shelf" is out of date (five books; the three Handling Data books arrive off) — next cut. (3) "Download CSV" copies to the clipboard and downloads nothing — the button should say Copy CSV — next cut (the film's caption names the button as it is and says what it does). (4) Two lessons for the recorder: a retake on a page whose store still holds the failed take's half-typed cells films "1313" (each answering chapter now starts from a clean book); Playwright's `video.saveAs` must be waited for (a rename raced the writer once).

**PROOF of the book-view cut at `635bebc`:** `run.js --fast` GREEN (bar qa-repo-prod until the record commit) · `run.js --full --book stats-averages`: sit-pupil 424 / sit-confused 199 / sit-teacher 199.6 GREEN at 375/768/1280 with the new law (qa-coverage red only for the other books' voided sidecars — the shared client changed; the whole-tree run is owed and runs after the films) · the teacher transcript re-extracted: hash unchanged (the header line is not in the transcript — a gap for the next harness session), qa-cold-read 19 GREEN · **LIVE: FRONT DOOR Version 38, 13 Sept 2026, 17:03** (server/DEPLOY_LOG.md; the served page carries the fix) · `control.js --only sit-teacher` for `bookview-wrong-book-name` — runs in the background after the films are cut (the audit says pending until its FIRED line is pasted).

## 13 SEPT 2026, 13:15 → 13:30 — THE SERVER CUT: the relay re-sends on the echo bounce, the store answers a stray GET with JSON, one open and one read per call (clock stamped 13:15 BST at `d7e437c`, budget 120 min; his ruling at 13:0x — "incorporate all fixes into this build. no more separate sessions. get it done here"; an opus package under the steward's brief, by file ownership)

**LIVE — DATA Version 2 (13:27) and FRONT DOOR Version 37 (13:30), both from `210ced7`, the same Code.gs in both projects, no manifest touched (server/DEPLOY_LOG.md).** A Code.gs change, so the eight-item smoke list is his.

**MEASURED FIRST (the steward, 13:09–13:10, a quiet moment, a class with no saved rows):** `hello` and `load` took 5.6–8.2 s of the store's OWN time (12 Sept: 1.4–3 s) — in the mock, one `hello` opened the Sheet 6 times and read the whole Data tab twice, a `save` 7 and 1, a `load` 5 and 1; the Data tab's cells carry whole jotter states, so each whole read pulls megabytes to answer a question about column A. And the relay's UrlFetch meets the same echo bounce the page meets (one answer in eight an HTML page), turning it into `relay-failed`; the store's `doGet` threw on the bounce's GET ("doGet Failed" rows).

**THE LAWS, written first and seen to fail on the unfixed tree (qa-two-homes):** L-A the relay re-sends ("the relay took the echo bounce for a closed road — one HTML answer and apiCall said relay-failed instead of asking once more"; control `relay-no-resend`), L-B the store's doGet ("the store answered a stray visit with a failure — the echo bounce lands here as a GET, and every one is a Failed row in the log"; control `data-doget-throws`), L-C one open, one read ("hello opened the Sheet 6 times and read the data tab 2 times in one call — measured 5.6–8.2 s a call on 13 Sept 2026 for a class with no rows"; control `scan-per-call`). The mock (`tools/qa/lib/mockenv.js`) gained counters only.

**THE FIX (server/Code.gs.template):** `RELAY_RESENDS = 1` — `apiCall` asks again once when the answer is not a 200 or not a JSON object, logging `relay: DATA answered <code> — <80 chars> for <action> (attempt n)`; `doGet` returns `{ok:false,error:'use-post'}` as JSON when `sheetId` is set (the standalone store) and touches nothing; `EXEC` — one execution's memo opened in `apiRelay` and cleared in its `finally`: `ss_()` memoised, `dataIndex_()` reads columns A–D once, `dataRowValues_(row)` reads one full row, `findRow_`/`haveAnyRow_`/`pupilCountByClass_`/`apiHello` use the index and read only the matching rows, `writeRow_`/`adminDeleteClass_` invalidate it, `getConfig_` memoised and `setConfig_` invalidates. Counts after: hello 1 open / 0 whole reads / 1 ranged; save 1 / 0 / 1; load 1 / 0 / 2. NOT changed, named: `apiSetName`, `adminWall_` (needs every pupil's Summary and Updated — a whole read is its cheap shape) and `adminDeleteClass_` still read the whole tab.

**PROOF at `210ced7`:** qa-two-homes GREEN 115 in both worlds; `run.js --fast` GREEN, 27 gates, qa-repo-prod included (at the clean tree); `control.js --only qa-two-homes`: 18 controls, every one FIRED (relay-no-resend, data-doget-throws, scan-per-call among them), over-tightening PASSES (`tools/qa/out/steward/controls-server.log`); qa-audit GREEN 230. Two reds on the way, fixed honestly: `dev/test-server-scoping.js`'s sheet stub had no `getValues` on a range (shape added; 20 of 20 pass); qa-scope measured the cut against `9197482` after the steward's record commit `d7e437c` landed in the same worktree — `startCommit` corrected to `d7e437c`, the clock left alone. The control `secret-in-a-return-value` CANNOT RUN once (the rewrite had dropped the guard its plant anchors on) — the guard put back, it FIRED on the re-run.

**LIVE NUMBERS (from outside Google, the page's own token; the store's own time in brackets):** a stray GET → JSON `use-post` in 1.5 s Completed (before: HTML, Failed). hello: before 6.7 / 36.8 / 10.4 s (5.7–8.2), after 4.4 / 4.0 / 4.0 s (3.0–3.9). load: before 10.4 / 6.3 / 7.0 s (5.6–8.2), after 5.8 / 5.4 / 8.4 s (3.3–4.8). What is left in a call is `openById` on a Sheet whose cells carry whole states, plus Google's own front (0–4 s on a bad round): the next cut can hold the index in CacheService and read a row by number.

**Left for the next cut, dated 13 Sept 2026:** the index in CacheService (a call should cost the row it needs, not the open); `apiSetName` and `adminDeleteClass_` onto the index; the outbox's 20 s cadence (no back-off); the relay's re-send is one — the bounce twice in a row is ~1 in 64 and still a `relay-failed`.

## 13 SEPT 2026, 11:19 → — THE STEWARD CUT: the set-up screen, the echo bounce, the film's caption (clock stamped 11:19 BST at `ec52b91`, budget 120 min; Fable steward, a background opus package for the set-up screen's code)

**WHAT HE FOUND (his first morning on Books A and B, a real pupil account; rulings 52–55 and the second continuation of 51 in MATHS_FEEDBACK_MASTER.md):** the three Handling Data tickboxes all read "Handling Data" (the markbook's book tabs too); the class table's controls wrapped and stacked raggedly; "open Set-up" named a screen that called itself "All classes"; the film's rhyme line said "Her own…"; and every screen waited — "Getting your details" 54 s, the markbook 40 s, the "still saving" card three times.

**MEASURED FIRST.** Set-up: the controls cell 113 px wide, "Open the markbook" 93×64 and "Copy link" 87×58 (two lines each), buttons at x = 10/16/16/16; the tickbox cell one non-wrapping flex line 966 px; the document 1,242 px wide on a 768 px screen and 1,221 on 375. The store: the front door's Executions log carried an `apiCall` (the relay) for almost every call, 4.4–22.7 s each; the DATA log a save re-sent every 20 s for over twenty minutes, each doPost Completed in 4–9 s (12 Sept: 1.4–3 s), one Running at 60 s, and "doGet Failed" rows among them. The direct path probed from outside Google with the page's own token, eight rounds: POST → 302 in 1.1–3.5 s, the echo GET 0.3–2 s and JSON — except one round in eight (two of three at 12:0x) where script.googleusercontent.com waited 15.5 s and answered a SECOND 302 back to the script's own /exec as a GET, i.e. an HTML page. **The echo bounce is Google's; treating it as a closed road was ours:** the page called it "not the store", took the relay (another doPost, 4–23 s), and for a save started the 20 s re-send loop — which is the load that turned 1.5 s doPosts into 4–9 s ones.

**THE FIX (client only, FRONT DOOR Version 36):** (1) `GJ.app.bookName` — a book whose title another shares is "Title · Volume", derived from ACTIVITIES; used by every staff naming site (set-up tickboxes, tick messages, bookTitle, class-page tabs, full-grid chooser). (2) The set-up table: series groups stack and wrap (`.ticks-group`), the controls cell is two lines the same in every row (`.row-acts`), the table scrolls inside `.ledger-host` with the hint "Scroll or swipe the table sideways to see all of it." only when width is hidden; the crumb reads Set-up here and on every crumb back. (3) `storeCall` re-sends the direct request up to twice on an answer that is not the store's JSON (`STORE_RESENDS = 2`), and only then the relay; the console names each re-send. (4) Book B s1 film step 9: his "Your own rhyme…" was a verbless fragment on the separated read, so the caption reads "Here is a rhyme to help you keep the four apart." (5) The separated reader also failed a bare "Set-up" crumb ("set up what?"), so the note above the class table now opens "Set-up is where you add a class and choose the books it sees." — the word stays (ruling 54) and is glossed at first meeting (checklist §6.3). (6) The table hint exists only while it is true: a hidden sentence was still read into the teacher transcript.

**THE LAWS, written first:** sit-teacher on set-up:classes at every width — distinct-book-names (control `tickbox-same-name`), one-line-controls (`row-control-wraps`), same-shape-every-row (no plant of its own), page-never-scrolls-sideways (`ledger-pushes-the-page` — its first plant, the frame alone, left the gate green because the wrapped groups fit a phone; the plant now puts the non-wrapping line back too); qa-waits laws 5–6 (`store-no-resend`). Every one seen to fail on the unfixed tree by its plant (the FIRED lines in the audit).

**PROOF at `9197482`:** `run.js --fast` GREEN (bar qa-repo-prod until the record commit) · `control.js --only sit-teacher`, three batteries: the first (12:03) tickbox-same-name FIRED, row-control-wraps FIRED, ledger-pushes-the-page DID NOT FIRE (the frame-only plant), over-tightening PASSES; the second (12:11, the widened plant) and the third (12:27, the final tree) all seven FIRED + over-tightening PASSES · `control.js --only qa-waits` (12:11): all eleven FIRED incl. store-no-resend, over-tightening PASSES · `run.js --full --book stats-averages` (12:06): sit-pupil 424, sit-confused 199, sit-teacher 199 GREEN · the whole-tree `run.js --full` (12:27–13:09, MS_WORKERS=4 beside the batteries): sit-pupil 572.6 s / sit-confused 230.9 / sit-teacher 200.1 GREEN, qa-coverage GREEN, every cell closed · qa-language 198 GREEN, qa-strings-ledger GREEN, qa-audit 230 GREEN · the cold read: the five changed sentences judged by path by a fresh separated reader (rows appended in TEACHER and stats-averages; two REWRITEs honoured — "Set-up" glossed, the caption made a sentence), TEACHER and v4 re-filed against the new transcript hashes with a dated note, qa-cold-read 19 GREEN. **LIVE: FRONT DOOR Version 36, 13 Sept 2026, 13:16 (server/DEPLOY_LOG.md).**

**SERVER SIDE — his ruling at 13:0x: "incorporate all fixes into this build. no more separate sessions."** So the server cut runs in this session as its own cut (its own clock and scope, an opus package by file ownership under the steward's brief): the relay re-sends once on the bounce and logs the answer; the DATA project's doGet answers a stray GET with JSON; one open and one read of the data tab per call (measured first: hello/load 5.6–8.2 s of the store's own time in a quiet moment, 13:09, against 1.4–3 s on 12 Sept). Its entry follows this one when it lands.

**Left for the next cut, dated 13 Sept 2026:** film captions are not in the pupil transcript at all (the s1 caption changed and the hash did not move) — the separated reader has never read a film; the tickbox labels and crumbs are not in the teacher transcript either (judged by path this cut);  the outbox's 20 s re-send cadence never backs off (a slow store is asked again every 20 s for as long as it is slow — a doubling to a two-minute cap, with its human-pace row); ACTIVITIES' titles and subs are printed on covers and now on the staff side and are in no strings ledger; Opus's done-list of 02:5x described labels and question numbers that were never on the page (its steps 1, 5, 6 and 12) — a checklist is read against the screen before it is sent.
...
## 12 SEPT 2026, 21:00 → — BOOK A, Handling Data · Collecting and displaying, AND THE DATA SPLIT (clock stamped 21:00 BST at `1bb9ea8`, budget 270 min = the class fixes 45 + Book A 180 + the split 45; RESUME_AB, Opus 5 orchestrating opus/sonnet packages)

**BOOK A IS LIVE — FRONT DOOR Version 34, 13 Sept 2026 00:27, from `3e16a9c`; the store is the standalone project's Version 1 (22:31). Proof rows and the direct-path timing in `server/DEPLOY_LOG.md`. Book A arrives UNTICKED for every class. `qa-manifest` post-deploy GREEN (18); `qa-repo-prod` green at the record commit. Proved at `3e16a9c`: `--fast` green; the whole-tree `--full` GREEN — sit-pupil 247/263/409/605 ×3 widths, sit-confused 142/147/154/202 ×3, sit-teacher 31, qa-coverage "every cell is closed" (movie:step-n now stood on deliberately); the separated cold read filed against the final hashes — Book A 101 PASS / 0 / 0, Book C 127 / 0 / 0, the shell 398 / 9 REWRITE / 2 FAIL (the two FAILs and the codes REWRITE are the standing declined rows: "Mark my working" IS qlist's Check label; the next point named is ruling 45; "KS3 · M2" is his department's code). The whole control battery runs in the BACKGROUND after this (`tools/qa/out/bookA/control-all.log`) and is reported in the audit when it ends, never waited for.**

**TIME, against the budget, line by line (wall-clock; the clock was stamped 21:00 at `1bb9ea8`, 270 min):** the class fixes 21:05–21:35 = 30 of 45 · the DATA split: code 21:04–21:17 (13) + Chrome 22:03–22:58 (55, of which ~25 waiting on the four pastes only he could make) = 68 of 45 — **over by 23, all of it hands-off waiting** · Book A: packages 21:04–21:25 then renderers/walks/fixes to the cut at 00:27, minus the split's Chrome time and the 25-minute rate-limit outage at 21:25–21:50 = ~165 of 180 · total 21:00 → 00:27 = 207 of 270 at the cut, ~225 with the record. Over budget on one line (the split, by waiting), reported here as the rule demands.

**What is NOT proved and is his:** the eight-item smoke list plus Book A's own line (place a leaf on the wrong stem, then press it once and see it stay); the shelf showing Book A the moment a class is ticked; a real pupil account through the new store (the deployer's visit proves the chain, never the pupil path — `reference_execute_as_user_sheet_rule`); the relay fallback observed twice tonight (DEPLOY_LOG note) — his console reads the reason if the cover ever waits ~25 s.

**Left for Book B's cut, dated 13 Sept 2026:** AV_*/RM_* dx detections in statcore (named in the design, not yet detected — Book B's `values` and `table` need them); the film-law plants `film-no-venn…lobf` and the six `stats-a-*` plants have their controls declared and the battery to prove them runs after this deploy; the old DATA deployment on the bound project to be archived (nothing points at it); the judge's shell REWRITEs beyond the declined three (pageFull/angleUnreadable "That" → the concrete noun; the teacher's "Show them" → "Show this pupil"; "Tick it on in Set-up"); the venn3 reserve question keeps its eight slots in the list under the drawing; INTERFACES/ADDING_A_TOPIC updated (DOCS-A).

**THE SPLIT (ruling of 12 Sept: "I want you to do all of this via the session").** Two Apps Script projects now: the bound **OLS — MathShelf** serves the FRONT DOOR only (manifest rests at `USER_ACCESSING` + `DOMAIN`, never edited again); the new standalone **OLS — MathShelf DATA** (`14j0H7VG…jn3h`) is the store — `ss_()` opens the Sheet by the `sheetId` script property, its manifest was set ONCE in the New-deployment dialog (`USER_DEPLOYING` + `ANYONE_ANONYMOUS`, read after the cut) and rests. The act whitelist is `acts_()`: the built-ins unioned with the Sheet's Config row `acts` (typed by the session: `["stats-collect"]`), so a new book is client-only plus one cell — `qa-tickbox` derives and PRINTS the row the live Sheet must carry from ACTIVITIES minus the template's literal (`mockenv.shelfActsRow`), and `qa-store-scale` mounts it. What was Damien's: allowing the session the deployment dialog, the OAuth **Allow**, pasting the relay secret into the new project (a value no session types), and pasting the new `/exec` into `dataUrl` (the desktop app refused the session that edit). The measured hop (PROGRESS 12 Sept, PACKAGE A) is unchanged by the split — the page still calls the store directly with its token; the split buys the manifest, not the speed. `qa-two-homes` 62 → 107 checks with a standalone world; 15 of 15 controls FIRED.

**THE CLASS FIXES (opened the cut, 21:05–21:35):** `.jq-margin` 13 px at ≤640 (the 11 px ink of q31/q32 at 375); the hollow follow-through tick at full green; the struck ghost keeps ink (the line through it is pencil); "Scroll or swipe the graph sideways…" (the judge's statScrollGraph FAIL honoured); the tray fiction now DEFINES the tray so it no longer says the boxplot stage instruction twice; a parked reading rule reads nothing (the stray "0" on Exercise 4's opening at 375); `fixture-css-amber` written for `qa-colour-law`'s declared control (COLOUR-TOKENS itself was already done: `--gold-ink` `#6F6108`, COLOUR_LAW reads its tokens); the theorem stamp 13 px / 700 (Book C's "How many were asked?" measured 4.45:1 on the whole-tree walk); six of the eight cold-read REWRITE/FAIL rows on Book C honoured in the pack (WORDS-C) and the two in jotter.js's banks by hand; q17's marking rule was already fixed by POLISH CUT 2 (RD31–34). NOT done: the 1,242 px phone chart with Join under it (decision (a) stands); the boxplot scale's 54 px laptop scroll IS done — statchart reclaims the question-number gutter on a laptop when the deficit fits (`data-reclaimed`), 54 → 0 px hidden at 1280, and the note and track stay in the text column.

**BOOK A.** Five kinds in `jotter-stats.js` — `order` (the data-handling cycle's cards), `pick` (the better questionnaire question + why), `stemleaf` (leaf tray → stem rows, back-to-back with a given side and prefilled rows, a two-tap key), `pie` (the angle table, then boundaries on the rim by tap and ±1° nudge, ✓ Draw the pie chart, labels onto sectors), `scatter` (points, a line of best fit on two dragged handles, the estimate by the cfread rule, correlation chips, the reading that looks wrong) — and `values` on a figure (Venn boxes ON the regions for two circles; a three-circle diagram lists its eight regions under the drawing). Engine (`statcore.js`, ENGINE-A): units, follow-through, model boards, gist per kind; `leastSquares`, `stemLeafOf`, `pieAngles`; 13 new dx; the ten `Q_*` reasons; selfTest **160 → 270**. Chart (`statchart.js`/`player.js`, CHART-A): `venn`, `pie`, `stemleafFilm`, the scatter line/handles/outlier, the y-axis break, eight film ops, all proved on a scratch page at 375/1280. Lint (LINT-A): the five kinds' re-derivations (Venn regions vs totals, integer pie angles, leaves reconstruct the list, least squares + Pearson r + the leave-one-out outlier test), 29 fixed FAIL stems proved on fixture questions. The pack (CONTENT-A): **5 sections, 32 questions (14 judge, 3 pick, 1 order, 5 values, 2 pie, 4 stemleaf, 3 scatter), 39 film steps, 87 marks; Core = 25.5 units of 26**; every scatter axis and the Q25 student-to-pair mapping read from the page (student 2 = (50, 5.2), student 3 = (30, 3.2)); the physical page numbers drift from the design's "Qn" from Q11 on (recorded). Decisions of mine: a scatter chart may declare `snap: 2` so a point can sit on a half small square (M7's 0.1 km data on 0.2 km squares — 60 squares tall, not 120); the LOBF "near" band is one small square OR twice the least-squares line's median residual (the true line must pass its own test); a judge's `judging` stage exists only when a claim that opens no reason picker is followed by another; a stemleaf with no key has marks [m, 0]; `SL_BACK_DIRECTION` is not a dx (the tap-first side grows outward by construction). Walkers (WALK-A): press routes for the five kinds incl. a real pointer drag for the line's handles; six `stats-a-*` plants; the walker waits up to 10 s for a long film step (31 leaves at 180 ms had it calling a film ended at step 6).

**Faults the walk found and fixed before anything shipped:** the Venn boxes' written labels collided at 375 (the region is the label; the dock names it; aria carries it); a box hanging past the frame hid 13 px; the venn3 boxes collided (its slots live in the list); the stem-and-leaf rows wrapped at 375 (nowrap + a sideways-scrolling host, the truth diagram too); the fiction line at 2.18:1; empty `<th>`/`<td>` on the pie table; a `[data-hit]` press target with `pan-x pan-y`; the pie rim's touch-action; `judging`/`ready` stages nothing could stand on; the trim() "(and N more)" row printed as "undefined — undefined"; a stemleaf row's empty `<td>` is the diagram's own statement (an exemption, stated in the law). Faults of MINE the lint found in the pack: dash-chains that were number ranges (the rule now ignores an en-dash between digits, with a MUST_FAIL probe and a MUST_PASS exemplar), three ASCII hyphens, a near-duplicate claim.
...
## 13 September 2026, evening — the staff-door cut (Version 39) and chapter 1 of the teachers' film re-cut

His ruling on the film's first minute: "one address" misleads (the staff link is the teacher's; every class carries its own link); "filmed on a practice class" is irrelevant; the first visit goes through Google's consent screen, where Select all must be ticked; and the film pressed a Staff button on a screen that already showed the staff passcode. Checking that last point against the tree found a live fault: `shell.css` gives `.btn-quiet` a display, a class rule beats the UA `[hidden]`, so `staffCover()`'s hide of the Staff button (and, by the same rule, the never-wired "Not you? Switch account" link) did nothing on the teacher's own door. Cut: `.btn-quiet[hidden] { display: none }`; law in `sit-teacher` (staff-cover:landing, every width); control `staff-door-stray-staff-button`; FRONT DOOR Version 39 at 18:43 from `f364310`, client-only, proof in server/DEPLOY_LOG.md. The 18:02 battery also reported `bookview-wrong-book-name DID NOT FIRE`: the walk switched to Algebra, where the planted v3 guess is right; the law now refuses to skip and stands a second time on a Handling Data book. Both controls: the 18:37 battery, pasted into MATHS_GATES_AUDIT.md when it ends. Chapter 1 re-recorded from the staff link's own door (no Staff press), six sentences through a fresh separated read (MATHSHELF_TUTORIAL_COLD_READ.md, the last section); Google's consent step is told in captions, not shown — it comes only on a real first visit with a real account. Teachers' film 26:15, copied to Claude Work/Maths/Tutorials; the pupils' film is untouched (chapter 1 is not in it).


## 13 September 2026, 19:08 — the gist cut (Version 40); the batteries

controls: green 2026-09-13 2ae5d45 — the sit-teacher battery: all nine controls FIRED, including `bookview-wrong-book-name` (corrected walk) and `staff-door-stray-staff-button` (the FIRED lines are in MATHS_GATES_AUDIT.md). The whole-tree `--full` started at 17:49 from `1f7c886` ended RED on two gates, both explained and neither an open fault: `sit-teacher` at 768 — the exercise grid's column gist at 4.2:1 in rendered pixels on a Handling Data book (the walk file was edited mid-run to prefer a third book, so the worker that started after the edit stood there first); fixed in `shell.css` (ink, weight 500), the 768 walk GREEN after it, live as FRONT DOOR Version 40 at 19:08 from `2ae5d45` (server/DEPLOY_LOG.md). And `qa-coverage` (2,636 "nothing stood on this question" lines across every book at 375/768) — the 18:37–18:46 control battery ran concurrently and wrote over the walk records in tools/qa/out while coverage read them; NOT re-run tonight, so the coverage picture at `2ae5d45` is unproved and owed to the next quiet --full. Rule for next time, learned twice today: one battery at a time in one tree.
