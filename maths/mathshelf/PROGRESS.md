...
**STATE AT THIS CHECKPOINT (23:2x): Book A built and walked green on its own cells at 375/768/1280; the standalone DATA project is LIVE and answering the front door (Version 1, 22:31; `dataUrl` flipped 22:5x; proof rows in DEPLOY_LOG); the FRONT DOOR cut for Book A is next (Code.gs + Index.html, one version, by Deployment ID). The done message below is written when it is live.**
