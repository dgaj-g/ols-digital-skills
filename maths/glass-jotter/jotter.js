...
(function () {
  ...
  function buildProtractor() {
    var R = 116, p = [], d, rad, x1, y1, x2, y2, len;
    p.push('<path d="M ' + (-R) + ' 0 A ' + R + ' ' + R + ' 0 0 1 ' + R + ' 0 Z" fill="rgba(15,107,102,0.10)" stroke="#0F6B66" stroke-width="1.6"/>');
    p.push('<path d="M ' + (-(R - 46)) + ' 0 A ' + (R - 46) + ' ' + (R - 46) + ' 0 0 1 ' + (R - 46) + ' 0" fill="none" stroke="#0F6B66" stroke-width="0.8" opacity="0.5"/>');
    p.push('<line x1="' + (-R) + '" y1="0" x2="' + R + '" y2="0" stroke="#0F6B66" stroke-width="1.4"/>');
    for (d = 0; d <= 180; d++) {
      rad = d * Math.PI / 180; len = (d % 10 === 0) ? 14 : (d % 5 === 0) ? 9 : 5;
    ...
    for (d = 0; d <= 180; d += 10) {
      rad = d * Math.PI / 180;
      var ox = Math.cos(rad) * (R - 24), oy = -Math.sin(rad) * (R - 24);
      var ix = Math.cos(rad) * (R - 38), iy = -Math.sin(rad) * (R - 38);
      p.push('<text x="' + ox.toFixed(1) + '" y="' + (oy + 3).toFixed(1) + '" text-anchor="middle" class="prot-num">' + d + '</text>');
      p.push('<text x="' + ix.toFixed(1) + '" y="' + (iy + 3).toFixed(1) + '" text-anchor="middle" class="prot-num inner">' + (180 - d) + '</text>');
    }
    ...
    p.push('<g class="rotate-handle" style="cursor:grab"><circle cx="' + (R + 17) + '" cy="0" r="12" fill="#0F6B66"/><path d="M ' + (R + 12) + ' -4 A 6 6 0 1 1 ' + (R + 12) + ' 4" fill="none" stroke="#fff" stroke-width="1.5"/><path d="M ' + (R + 9) + ' 3 l 3 4 l 3 -3 z" fill="#fff"/></g>');
    return p.join('');
  }
  ...
  function mountProtractor(host, q, savedRec, hooks) {
    ...
    svg.style.cssText = 'touch-action:none;max-width:520px;width:100%;display:block;margin:0 auto';
    body.appendChild(svg);

    var V = q.vertex || [248, 206], L = q.armLen || 132;
    ...
    var handle = prot.querySelector('.rotate-handle');
    if (handle) handle.addEventListener('pointerdown', function (e) { e.stopPropagation(); onDown(e, 'rotate'); });

    body.appendChild(el('p', 'ui-msg', 'Line the small red centre mark on the corner, turn the protractor so 0 sits along one arm, then read where the other arm crosses. Use the rotate knob on the right to turn it.'));
    var compHost = el('div', '');
