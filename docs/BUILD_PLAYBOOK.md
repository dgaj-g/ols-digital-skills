...
7. **You are the QA, not Damien — sweep the WHOLE app with your own eyes before he ever sees it.** This is non-negotiable and it is where builds keep going wrong. Before you show Damien anything, you must run a **comprehensive, sweeping visual review of the entire app** — every screen, every state, every breakpoint — and fix every alignment slip and common-sense visual problem *yourself*. Damien repeatedly having to point out things you should have caught (a symbol that looks wrong, a character too dim against its background, a misaligned track, a column dwarfing its content, a bubble that overflows) is a **failure of the build**, not a normal review step — it wastes his time and it is the single biggest source of friction. Treat "would I be embarrassed to put this on a Promethean board in front of an inspector?" as a question you must answer *for every screen* before handing over. The full method is **Step 8b — The full visual sweep**, and on an ultracode session it is a multi-agent pass; do it without being asked, every build.

8. **Think it through and make the sensible call yourself — the build is judged as a whole experience, not a checklist of parts that each "work".** Before you commit to a mechanic, follow its premise all the way to the end and ask what it needs to actually land **across every scene, not just the showcase ones**. A concept whose core promise your *own* content decisions quietly undermine is broken however polished the pieces — and the gap reads as unfinished, not as a clever fallback. The rule this came from: a camera / "bring the blurred **photo** into focus" build shipped with **10 of its 16 scenes having no photo at all** (they'd been dropped to plain text cards because the images were hard to source) — so the central trick was strong on the 6 photo scenes and **hollow** on the other ten, where a big "OUT OF FOCUS" viewfinder wrapped around bare text. That should never have shipped, and Damien should never have had to point it out. Concretely:
   - **If a mechanic depends on a media type** (photos, audio, video, a 3-D model), confirm **up front** you can source *enough* of it to honour the concept — most of the set, not a handful. If you can't, **change the mechanic, or make the non-media case just as deliberate and strong** (its own visual identity, its own honest framing) — never a bare "nothing to see here" fallback dressed in the media mechanic's clothes.
   - **When you notice an obvious "this would bug me if I were the pupil or teacher" problem, fix it on your own judgement.** Imagine a pupil playing the *whole* thing start to finish, and a teacher demoing every screen on a board — anything that would make you wince is a defect to resolve now, silently, not to ship and explain.
   - **Reserve questions for genuine forks only you can't resolve.** "Should most scenes be photos or is an empty viewfinder fine?" is not a fork — a moment's thought settles it. Decide, do the sensible thing, and flag the decision in the PR; don't outsource judgement you're equipped to exercise.
...
8. **Match the model to the phase** — at two levels. (a) **Per session** Damien picks the main model: top-tier for design judgement and tricky debugging (Fable 5.1 designs, Opus 5.5 builds — point 9); a mid-tier model is fine for mechanical follow-the-handover work (capture sessions, asset processing, registry updates) and burns the allowance far slower. (b) **Within a session**, regardless of the main model, you still route delegated grunt-work down a tier per point 3 above — the two compose (Opus main thread + Sonnet/Haiku subagents is the cheapest way to run an ambitious build).
9. **THE STANDING SPLIT — Fable 5.1 designs, Opus 5.5 at MEDIUM builds (Damien's rulings 8 Aug 2026 "this is the way we work" and 23 Sep 2026; KS3 DT master file rules 158 and 283–288; the BUILD STANDARD loaded into every window from `~/.claude/CLAUDE.md`).** The design window is **Fable 5.1 at xhigh**: it thinks it through, decides, and writes the binding deliverables — the spec, the prototype at final size, the judging routines with their controls, every pupil-facing string, the acceptance tests and the build prompt — applying no fixes and writing no repo code. The build window is a **fresh window on Opus 5.5 at medium** (high only when Damien names a hard problem; never xhigh or max, where Opus 5.5 thinks more per turn than Opus 5 did and out-spends Fable): it executes the spec mechanically under the BUILD STANDARD meter — gates before surfaces, then plumbing, wiring, packing, deploy. Readers are Sonnet (the `cold-reader` agent, one packet per call). The spec files are the contract, so a usage cutoff between the phases loses nothing. Fable also flags when a task is too mechanical to need it at all. Held trial (23 Sep 2026): a REPEAT of an already-built pattern may design on Opus 5.5 at high when Damien's first message names it; a new pattern stays on Fable.
10. **A NEW user-facing pattern runs `/build` twice.** First run, on Fable 5.1 xhigh: `/build <N> DESIGN WINDOW ONLY …` — Steps 0–6 of this playbook (orient, fetch, parse, download, read everything, restate and wait, survey the gallery and choose the pattern), then the design deliverables into a design folder under Claude Work (`<Department>/<Activity>/`: COVERAGE.md, SPEC.md, prototype/index.html, judges with controls, STRINGS.md, ACCEPTANCE_TESTS.md, BUILD_PROMPT.md, PROGRESS.md). No branch, no code in the repo, no PR. It ends by handing Damien the five prototype-look questions (`KS3 DT Platform/PROTOTYPE_LOOK_CHECKLIST.md`) and putting BUILD_PROMPT.md on his clipboard (`LC_CTYPE=en_US.UTF-8 pbcopy`). Second run, a fresh window on Opus 5.5 medium: `/build <N> ` followed by the pasted BUILD_PROMPT.md — Steps 7–11 (build, test, PR, notify, stop), gates first. A REPEAT of an existing pattern skips the design window and builds straight from the existing spec in one run.
...
4. **No ancestor of a draggable element may create a containing block for `position: fixed` — this is the one that shipped broken to Damien (Terra Mobilis, Aug 2026).** The drag lift works by setting `position: fixed` plus `left`/`top` taken from `getBoundingClientRect()`, i.e. **viewport** coordinates. That is only correct while nothing above the element is a fixed-position containing block. Any ancestor with `transform`, `filter`, **`backdrop-filter`**, `perspective`, `contain: paint/layout/strict/content`, or a `will-change` naming those becomes one — and then those viewport coordinates are measured from *that ancestor* instead. The chip leaps away from the cursor the instant you start dragging and never comes back. The real case: a sticky word-bank styled `backdrop-filter: blur(6px)` for a frosted look. **Headless Chrome does not paint backdrop-filter, so every scripted test passed** while the activity was unusable with a real mouse on a real machine. Two defences, use both: (a) never put those properties on a container that holds draggables — use an opaque background instead of frosted glass; (b) after the lift, re-measure the element and carry the difference as a correction, so a future CSS change cannot silently resurrect it (see `geography/a2-plate-tectonics/assets/drag.js`).

5. **Never store drag bookkeeping on a reserved DOM property.** A draggable element's `.slot` is a *native* `HTMLElement` property (a string, for shadow-DOM slotting). Writing `tile.slot = slotElement` silently stringifies it to `"null"`/`"[object HTMLElement]"`, so your placement logic reads a string and the drop silently fails (the Isotope Lab mass-spec builder shipped broken this way once). Use a clearly-custom key (`_slot`, `__home`, `dataset.*`) for any element bookkeeping — never bare `.slot`, `.value` on non-inputs, `.label`, `.title`, `.name`, etc. (NB synthetic-pointer QA also can't drop onto a target scrolled outside the viewport — `elementsFromPoint` returns null there — so give the page a tall viewport before drag-testing.)

**Prove the drag with REAL input before you hand it over — synthetic events cannot do it (MANDATORY on every drag activity).** `dispatchEvent(new PointerEvent(...))` does not engage pointer capture, hit-testing, native selection/drag, or the compositor. A drag engine can be comprehensively broken for a human and pass every synthetic check you write — that is precisely how a build reached Damien with dragging that did not work on a mouse. Drive a real browser instead: **`geography/a2-plate-tectonics/tools/drag-harness.js` is the reusable harness** (Puppeteer, real `page.mouse` and real CDP touch events). Point it at any activity with `--url`, or copy it. It walks every drag activity in the build and, for both a mouse and a finger, asserts that the chip **lands in its target**, that **the grabbed element stays under the cursor** (the assertion that catches the containing-block bug — pixel drift, not a CSS proxy), that there is no transition or animation mid-drag, and that the page-wide selection lock goes on and comes off. It also statically flags any ancestor of a draggable that would create a fixed-position containing block, because headless rendering can hide the behavioural symptom. Run it, get it green, and only then show Damien. Watch it work with `--headful`.
...
1. **The dragged element must track the pointer 1:1, instantly — zero lag.** The single most common cause of "the dragging is laggy" is a **CSS `transition` on `transform`** (e.g. a base `.chip { transition: transform .1s }` for hover): while dragging, every `pointermove` then *animates* the element toward the pointer over that duration, so it visibly chases the cursor/finger. **Kill the transition on the dragged element while it is dragging** — `.chip.dragging { transition: none; }` — this is the fix. Then: set the move with a GPU-composited `transform: translate3d(dx,dy,0)` (add `will-change: transform` on `.dragging`), applied **synchronously on every `pointermove`** so the visual never waits. **Do NOT do heavy work synchronously in `pointermove`** — `elementsFromPoint`/`closest` hit-testing and hover-class writes are expensive and, run on every event, they stall the paint and the element lags. **Coalesce the hit-testing to once per frame with `requestAnimationFrame`** (store the latest x/y in `pointermove`, do the hit-test + hover highlight in the rAF callback; cancel it on release). Reference: `irish/caitheamh-aimsire/script.js` (`onMove` + `hitTest`). Verify by reading `getComputedStyle(chip).transitionDuration` mid-drag — it must be `0s`.
   - **Kill any CSS *animation* on the dragged element too, not just the transition — `.dragging { animation: none !important; }`.** A still-running keyframe animation (most often an `@keyframes` *entrance* like `cardIn`, but any looping/`both`-fill animation counts) keeps writing `transform` every frame and **overrides your inline `translate3d`** (the Animations cascade origin beats inline styles), so the element ignores the pointer and won't track — it keeps doing its entrance instead. This bit *Ceart nó Mícheart* (a single large `.cm-card` that animates in on every new card, so pupils routinely grab it mid-entrance); the chip-tray reference never hit it because tray chips have no entrance animation, making it easy to miss on any build where the dragged thing also animates in. Also drop the entrance's `fill-mode: both` (a filled final `transform` lingers in the same origin). Verify `getComputedStyle(el).animationName === 'none'` mid-drag. Reference: `irish/ceart-no-micheart/style.css` `.cm-card.dragging { transition: none !important; animation: none !important; }`.
...
3. **Keep the draggable items reachable from their targets — everything usable without long-distance dragging.** A long list of source items parked in a tray *below* a tall grid of targets means a pupil must drag from the bottom row all the way up to the top row (and fight the scroll). Don't ship that. Options, pick what fits: **(a) a sticky/always-in-view word bank** (`position: sticky; bottom: 0`) so a source item is always a short drag from whatever target is on screen — the Caitheamh Aimsire Match solution; **(b) a compact-enough layout that the whole board + tray fit one screen**; or **(c) rounds** of a handful of items at a time. Combine with edge auto-scroll during drag so off-screen targets are still reachable. **Drive that auto-scroll from a *persistent* `requestAnimationFrame` loop that runs for the whole drag, not from the `pointermove` handler** — otherwise it stalls the moment the finger stops moving, so a pupil who parks their finger at the bottom edge to bring an off-screen target into view gets nothing (the card just sits there, reading as "stuck"/unfluid). `pointermove` only updates the latest x/y; the loop reads them each frame and scrolls. Reference: `irish/ceart-no-micheart/script.js` (`loop()` started on lift, cancelled on release). Make the targets generous (rule 2 helps). Verify at 375 px and at 1280 px+ that no item requires an awkward long drag.
...
### Judgement / verification ("do these agree?") — reference: `irish/ceart-no-micheart/`
The pupil is shown a **claim** (an item paired with a label or proof) and must **judge whether it is correct** — Ceart/Mícheart, true/false, approve/reject, real/forged — rather than "find the match". This **inverts the verb** most match/sort/quiz patterns use, so it is a strong way to make a familiar topic feel like a genuinely new game (it was chosen specifically to differentiate from the five existing Irish "find-the-match" builds). Reference build *Ceart nó Mícheart?* shows an Irish place-name + a "proof" (a national **flag**, **GAA county colours**, or an **English name**) and the pupil judges whether they name the same place; both correct and incorrect verdicts teach. **Genuine consequence is structural, not bolted on:** mismatches pair a confusable look-alike (Italy/Ireland flags; cross-cluster county colours — never a colour-twin from the same cluster), the match:mismatch ratio is **balanced and shuffled** (so neither always-Ceart nor always-Mícheart works), and 3 lives give a real fail state. **The verdict must never leak before commit** — the spoken audio plays the visible *label*, not the answer, and nothing in the DOM/aria reveals match-vs-mismatch. The card is judged by **drag-to-zone** (Pointer Events; freeze the stage height on lift so the verdict zones can't shift under the finger mid-drag — a real mobile bug caught in QA), by click, or by arrow keys. Reach for this when a topic has verifiable name↔attribute links and you want a shape pupils haven't met.

### Organise / build-a-structure ("populate the grid") — reference: `irish/clar-ama-beo/`
The pupil **populates a persistent 2-D structure** (a timetable, calendar, chart, layout) and a single **"go live" batch commit** grades the whole thing at once — a different verb from match / tap / judge, which all act on one item against one target. Reference build *Clár Ama Beo* ("the Living Timetable"): pupils hear Róisín announce a school subject (audio = the only prompt; the lesson cell shows no text) and drag its colourful picture into that lesson, building a weekly timetable day by day; "Cuir an Clár Ama Beo!" reveals which classes turn up right and which are wrong (lose a life). Genuine consequence: the answer is carried only in the audio, the bank tiles are **pictures with no visible text** and are **silent** (so there is no audio-to-audio or audio-to-text word-match bypass — each tile's *accessible name* is the English **meaning**, never the Irish word, so a screen-reader/inspecting pupil still has to know the heard Irish), the bank shows all options (no elimination), and there's a real fail state (3 lives; "go live" disables after a wrong check until a wrong cell is re-placed, so the pupil can't drain lives by re-checking unchanged answers). It also turns near-synonyms into the teaching (Corpoideachas in a class period vs Corpoiliúint as training; Eolaíocht umbrella vs the three sciences). Mobile shows **one day/column at a time**; wide screens show the whole week filling up. Reach for this when the topic has a natural structure (timetable, calendar, process, layout) and you want pupils to *assemble* it, not just match pairs.
...
## Step 8b — The full visual sweep (MANDATORY before Damien ever sees the build)

**This is the most important quality step and the one that keeps failing. Do not skip it, do not rush it, and do not hand the build to Damien until it is genuinely done.** The checklist above proves the activity *works*; this step proves it *looks right*. Damien is **not** your QA. Every time he has to point out something you could have seen yourself — a symbol that looks wrong, a character too dim to read, a track line that doesn't go through its dots, a column dwarfing its tiny contents, a speech bubble wrapping into a column or spilling off-screen — that is a **defect you shipped**, and the back-and-forth it causes is the single biggest waste of his time. Eliminate it here.

**The mindset:** look at every screen and ask, brutally, *"would I be embarrassed to put this on a Promethean board in front of an ETI inspector?"* If any part looks off, amateurish, cramped, lopsided, dim, misaligned, or just "not quite right" — **fix it now, before he sees it.** "Technically works" is not the bar; "looks deliberately designed" is.

**Capture every screen × every state × every width, look at the actual pixels, and fix everything:**
- **Screens & states:** the hub/landing; every game screen; every result/feedback state (all-correct, partial, wrong, and any "time's up"/end state); any modal, overlay, intro, or transition. Drive the activity into each state (simulate the interactions) and screenshot it — don't assume, *look*.
- **Widths:** 360–375px (phone), 768px (tablet), 1280px (laptop), and 1920px (board). Resize and screenshot each. Things that look fine at 1280 routinely break at 375 and look lost/empty at 1920.

**What to hunt for (these are the exact categories that have caused repeated rework):**
1. **Alignment & centring.** Everything that should line up, lines up: connectors/wires meet their anchors exactly; a track/progress line runs through the *centre* of its markers and the moving piece lands *on* each marker; speech bubbles sit centred over their trigger; stat rows and number/label baselines align; icons are centred in their tiles. Sub-pixel and "3px off" count — they read as sloppy.
2. **Balance, spacing & proportion.** No element dwarfs its content (a huge card around a tiny symbol), no column towers awkwardly over the other, padding is consistent, and there's no big dead whitespace or cramped collision. If a column is mostly empty around a small image, tighten it.
3. **Images/icons/characters on their actual background — brightness AND correctness.** Any image must be clearly **visible and bright enough against the surface it sits on** (a cartoon drawn for white will *disappear* on a dark theme — fix by lifting the artwork/giving it a light pool, per the activity's own notes/memory, not by hoping). Any **icon or symbol must look RIGHT**: recognisable, correct, properly contained within its frame, not crude or hand-drawn-looking. (Reach for real/official artwork over hand-drawn SVG — see Step 7 / the memory on activity images.)
4. **Overflow, wrapping & clipping.** Nothing spills off-screen or out of its container at any width; long text wraps cleanly inside cards; labels/bubbles/pills don't clip, get cut off, or wrap into ugly 1-word-per-line stacks.
5. **Colour & contrast.** Text is readable on its background everywhere; small coloured text isn't washed out; nothing relies on a colour that's invisible in bright ambient light on a board.
6. **Motion & polish.** Animations are smooth and intentional (no jank, no element fighting another, no leftover effect — e.g. confetti — bleeding onto the next screen); focus states are visible; hover/press feedback feels right.

**On an ultracode session this is a multi-agent pass, and you run it without being asked:** spin up parallel reviewers — a CSS/HTML **alignment & spacing auditor**, a **contrast/brightness measurer** (measure images vs their real backdrop colour, with numbers), and a **render/"does this look right" critic** — *and* do your own screenshot sweep across every screen and width (the agents reason from code; only your screenshots see the pixels). Synthesize, fix **everything** they surface, then **re-screenshot to confirm the fixes** (a fix often introduces a new issue). Only once you have personally looked at every screen at every width and there is nothing left that you'd be embarrassed to show, do you move on to the PR.

**Then, and only then, show Damien.** In the PR/notes, it's fine to say "I swept every screen at every width and fixed X, Y, Z" — but never make him find the things the sweep is for.
...
The Co-Authored-By line names the model that actually did the build (Opus 5.5 under the standing split; Fable 5.1 if Damien ran the build there).
