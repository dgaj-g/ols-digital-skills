...
The reference build is an **editable / buildable** variant: instead of a fixed base image it lays out clickable nodes (three branch cards in a checks-and-balances triangle on wide screens, collapsing to a stacked list + chip grid on phones). Each node opens a side drawer where pupils write their own notes and add real-life examples — auto-saved to `localStorage`, exportable to a JSON file, and shareable by a **link that carries the whole diagram inside the URL** (state is `JSON → gzip via CompressionStream → base64url`, with a plain-base64 fallback for old browsers; the receiver's `init()` decodes the `#d=` hash, persists locally, then cleans the URL). A one-click "Model answer" fills every box with content built from the teacher's source material (so the same activity is a blank build-it-yourself tool AND an answer key), and a separate "Test yourself" mode is the genuine-consequence quiz. Reach for this pattern when a brief wants pupils to *construct* a labelled diagram over time, not just read a fixed one. Pair it with a collaborative Google Slides deck when true real-time multi-author editing is needed (a static site can't do live collaboration).
