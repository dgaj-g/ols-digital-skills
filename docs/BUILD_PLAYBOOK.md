...
9. **THE STANDING SPLIT — Fable 5 thinks, Opus 5 executes (Damien's instruction, 8 Aug 2026: "this is the way we work").** For substantial design/evaluation/diagnosis work he puts the session on **Fable 5**, whose whole job is judgement: think it through, decide, and write a **binding spec file** (executed exactly, no re-drafting) plus log his rulings — **applying no fixes**. Fable says plainly when the spec is ready; Damien switches the same session to **Opus 5 (High)** and says "go"; Opus executes the spec mechanically — code, harnesses, builds, deploy walkthroughs. The spec file is the contract, so a usage cutoff between the phases loses nothing. Fable also flags when a task is too mechanical to need it at all. (KS3 DT master file rule 158 records the ruling; it applies to every project.)
