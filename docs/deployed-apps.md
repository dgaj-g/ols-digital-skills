# Deployed login-gated apps — `/exec` URL registry

A lookup of the **live Google Apps Script web-app URLs** for our login-gated activities
(the ones served from Apps Script, not GitHub Pages). These `/exec` URLs are created by the
teacher at deploy time and can't be known by Claude otherwise, so they're recorded here:
**any future session can `git pull` and answer "give me the link for the … activity".**

The `/exec` URL is **not secret** — the within-domain Google sign-in gate protects the data,
so an outsider with the link still can't get in. Safe to keep in this public repo.

When a login-gated activity is (re)deployed and the teacher pastes the new `/exec` URL,
update its row here. If ownership is transferred to another teacher, they redeploy and the
URL changes — update it.

| Activity | What it is | `/exec` URL | Deployed by | Last updated |
|---|---|---|---|---|
| Girls Coding with Confidence — Computational Thinking Challenge | Login-gated Bebras-style timed competition (40 min, 20 questions); named class boards, per-class QR, teacher dashboard + stats | https://script.google.com/a/macros/c2ken.net/s/AKfycbxo_jz7qYI4dlfA_onu-1dEqPgXv7vw2DOxpf6-pkoS-TV-BAlJiq01xAES1PJc6mqQ/exec | dgartland021@c2ken.net | 2026-06-06 |
| Government & Politics — The US Constitution Diagram | Login-gated collaborative class board (build shared notes) | https://script.google.com/a/macros/c2ken.net/s/AKfycbyz_Hp_XE5RbzYPYZM4EGtWh49pMLgVyhDnsE1udpDKDatwbQuDACRoDJlc9j3WpUQ/exec | Gov & Politics (c2ken) | 2026-06-06 |
| French — Mon Carnet de France (J1 "La Belle France") | Login-gated project builder: 4 stations → generates the pupil's project as a Google Doc in HER OWN Drive (Execute-as-user), auto-shared to her class's teacher, filed into OLS Digital Skills/French/J1; multi-teacher staff panel + dashboard; Docs + Sites guide decks | https://script.google.com/a/macros/c2ken.net/s/AKfycbzsFmN2Cja5GaKKrqs9Covbw2V_hkPfTLGrSgbmB4lAiL8FRQuhkzq5jHUNZDzyYIROjw/exec | dgartland021@c2ken.net | 2026-06-10 |
| Chemistry — Isotope Lab (Isotopes 1.1.10–1.1.12) | Login-gated S1 activity: Build-an-Atom (3D, Three.js), Isotope Snap, Mass Spectrometer; **auto-reads each pupil's name from C2k** (execute-as-user, no typing); class leaderboard, per-pupil dashboard + CSV, collaborative groups (teacher-controlled hidden/reveal). Source: `chemistry/isotope-snap/` (+ `server/`) | https://script.google.com/a/macros/c2ken.net/s/AKfycby4JnZqlrrATt9llI1dGeZX6c1x3n-fTPeC7rGzmzvgJCr5JtNVIsIDwpYZ-GVjv-NNGg/exec | dgartland021@c2ken.net | 2026-06-15 |
| KS3 Digital Technology Platform (J1 Block 1, Lessons 1-5) | The KS3 DT teaching platform itself: login-gated lessons with badges/XP, auto-paired Comms Channel activities, baseline + recap engine, and a full staff panel (classes, per-lesson locks, Live/Absence/Teams/Options/Cover, teacher briefs, Press Night + Pairing lenses). Content is fetched from github.io, so new lessons need a push, not a redeploy. Source: `ks3-dt/` | https://script.google.com/a/macros/c2ken.net/s/AKfycbw3u7k5DvIx01zGKBS1vFIIQuH_G4WS_vhLQrRWQiovOwEZEPPijWX_Zgrjm--BxGqUSg/exec | dgartland021@c2ken.net | 2026-07-28 |

| Home Economics — My Kitchen Portfolio (KS3 Digital Portfolio) | Login-gated portfolio builder: per-practical reflections (year-aware J1/J2/J3 focus areas), phone photo/video upload into the pupil's own Drive, one appending portfolio Doc per pupil auto-shared with her class teacher; multi-teacher staff panel with mandatory year group per class, dashboard + CSV | https://script.google.com/a/macros/c2ken.net/s/AKfycbxFtOSNATJUpwr40wMUGiyjIKD1dlgib9TfgGkR0BznveErhnDrE-U8_m-wHPNK8pO55A/exec | dgartland021@c2ken.net | 2026-06-12 |

**Notes**
- The teacher reaches the **bare `/exec`** for the Staff/admin landing; pupils use a **per-class link** (`…/exec?class=NAME`) that the teacher generates and shares from inside the activity's admin panel.
- Source code: GG lives in Claude Work `0. Digital Skills Web Activities/GG/`; the Constitution Diagram lives in `government-politics/constitution-diagram/` (+ a server package in Claude Work); Mon Carnet de France lives entirely in this repo at `french/mon-carnet-de-france/` (activity + `server/` + `HANDOVER.md`).
