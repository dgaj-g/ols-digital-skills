...
| `engines.js :: transcript :: 1100` | `PairKit.RELAY_GAP_MS`: how long the paced send queue leaves between two channel messages from the same pupil. | It bounds a MACHINE, not a pupil, and it exists because the server refuses a second message inside one second (`doPairSend` / `apiPairSend`) and refuses it SILENTLY — which is how his partner's second question and every printed line went missing on 28 Aug. 1,100 ms clears the server's own one-second window with room to spare; nothing she does waits on it, because the beats are relayed while she is already typing the next answer, and a beat is never dropped, only delayed. |
| `engines.js :: lines :: 180000` | `PairKit.WAIT_HINT_MS`: how long a pupil waits for a partner before the card offers her a way on. | Three minutes of a sixty-minute hour, and the waiting card is honest and moving for every second of it (DFM 100). It is a FLOOR on a hint, never a cut-off: nothing is taken away when it fires, a way out is added — and since this round the way out is the one the hint actually names, a button that tests her own bot (S12). A harness winds it forward rather than waiting three real minutes, which is why it is a named constant and not a number in the middle of a function. |
