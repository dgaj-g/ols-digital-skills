...
(function (global) {
  ...
  /* ================= L4 — THE JUDGING ROUTINES FABLE WROTE (K44 / DFM 283) =====
     Lifted VERBATIM from KS3 DT Platform/L4_PROTOTYPES/probes/{run-py,paths,
     calls,relay-doors}.js, where they were proved against the platform's own
     Skulpt with 26 controls (§C14). The only substitution the contract allows
     is the runner: `runPy` was a minimal Skulpt runner with EXACTLY PyRun.start's
     shape, and here it IS PyRun.start, wrapped so the three routines keep the
     signature they were proved with — `runPy(code, { inputs, ask, onOut,
     limitMs, preamble, epilogue })` resolving `{ ok, out, err, asks }`. Nothing
     inside checkPaths / checkCalls / relayDoors was edited: the controls that
     proved them (qa-paths-probe, qa-calls-probe, qa-relay) run these functions
     off this object, so a drift here would fail there.
     WHAT `asks` IS FOR: every input( ) the program made, with the prompt it
     showed and the length of `out` at that moment — which is what lets a probe
     say what was printed BEFORE the question (the scene) and what came after
     (the road). An input with no answer left in the queue resolves '' (the
     program carries on with an empty answer), the honest way to observe "asked
     more questions than it should" without hanging. */
  PyRun.runPy = function (code, opts) {
    opts = opts || {};
    var inputs = (opts.inputs || []).slice();
    var out = '';
    var asks = [];
    var run = PyRun.start(code, {
      ...
      inputfun: function (prompt) {
        ...
        return opts.ask ? Promise.resolve(opts.ask(String(prompt == null ? '' : prompt)))
                        : Promise.resolve(inputs.length ? inputs.shift() : '');
      }
    });
    var p = run.p.then(function (r) {
      return { ok: !!r.ok, out: String(r.out || ''), err: String(r.err || ''), offline: !!r.offline, asks: asks };
    });
    p.abandon = run.abandon;
    return p;
  };
  ...
  PyRun.DOOR_RX = /^NEXT: door ([AB])\s*$/;
  PyRun.parseRoomRun = function (res) {
    var DOOR_RX = PyRun.DOOR_RX;
    function tidy(s) {
      return String(s == null ? '' : s).replace(/\r/g, '')
        .split('\n').map(function (l) { return l.replace(/[ \t]+$/, ''); })
        .join('\n').replace(/\n+$/, '');
  ...
  /* WHICH REPLIES MUST COME BACK OUT (14 Sep 2026, j2-04's name-the-player job).
     `usesReplies: true` means EVERY typed answer must appear in what was
     printed — the Lesson 3 chatbot rule, where a bot that ignores what you said
     is the fault. A room with a fork in it never echoes the CHOICE word (it
     chooses a road), so on a card that asks a name and then a choice the rule
     as written could not be passed by any pupil — the expert walk answered
     both questions for ever. `usesReplies: [0]` names the replies that must
     echo by position; `true` still means all of them. */
  PyRun.repliesToEcho = function (want, replies) {
    if (!want) return [];
    if (Array.isArray(want)) return want.map(function (i) { return replies[Number(i)]; }).filter(function (r) { return r != null; });
    return replies.slice();
  };
  PyRun.checkPaths = function (code, cfg, runPy) {
    runPy = runPy || PyRun.runPy;
    var parseRun = PyRun.parseRoomRun;
  ...
  PyRun.CALL_MARK = '@@L4CALL@@';
  PyRun.callsEpilogue = function (fn, args) {
    var MARK = PyRun.CALL_MARK;
    ...
    args.forEach(function (a, i) {
      ...
      /* A NAME MISSING INSIDE THE MACHINE IS NOT A MISSING MACHINE (14 Sep 2026,
         qa-l4-cards, DFM 197 — recorded, and the one edit to the lifted routine).
         As proved, every NameError on the call read as NOMACHINE, so a pupil who
         typed the slot's name wrong INSIDE her machine was told "the floor cannot
         find a machine called price — check the name on your def line", which is
         the wrong line to look at. Python's own message names the name that is
  ...
  PyRun.parseCallMarks = function (out) {
    var MARK = PyRun.CALL_MARK;
    var got = {};
    String(out || '').split('\n').forEach(function (l) {
  ...
  PyRun.checkCalls = function (code, cfg, runPy) {
    runPy = runPy || PyRun.runPy;
    var MARK = PyRun.CALL_MARK;
    function display(m) {
      if (!m) return '';
    ...
    var epi = PyRun.callsEpilogue(fn, args);
    ...
    return runPy(code, { epilogue: epi, limitMs: cfg.limitMs }).then(function (r) {
      ...
      ref = r;
      var mine = PyRun.parseCallMarks(hers.out), want = PyRun.parseCallMarks(ref.out);
  ...
  PyRun.relayDoors = (function () {
    ...
    return { nextRoom: nextRoom, planPlay: planPlay, seeded: seeded };
  })();

  /* ---- L4 SURFACES SHARED BY THREE CARDS (§C13): the path table, the factory
     floor with its reject bin, and the bloat meter. Lifted from the prototype's
     paintTable / renderFloor / j3m1 with every pupil-facing word made CONTENT
     (the prototype hard-coded a handful — "Your product", "… and N more like
     these", the all-None sentence, "Python stopped:" — and a new engine sentence
     no gate reads is exactly the debt ENGINE_STRINGS_DEBT.md exists to stop
     growing, so those ride in the lesson's own `floor` / `pathTable` blocks under
     the keys below and go through the language gate like every other sentence).
     The grouping rule is Fable's: three pairs in the bin, then "and N more like
     these"; every product None collapses to one sentence naming return. */
  PyRun.pathTableHtml = function (T, r, opts) {
    T = T || {};
    opts = opts || {};
    var stoppedLead = String(T.stoppedLead || 'Python stopped:');
    return '<div class="pth"><h3>' + esc(T.title || '') + '</h3><p class="pth-lead">' + esc(T.lead || '') + '</p>' +
      '<table><thead><tr>' + (T.cols || []).map(function (x) { return '<th>' + esc(x) + '</th>'; }).join('') + '</tr></thead><tbody>' +
      (r.rows || []).map(function (row) {
        var word = row.kind === 'stranger' ? String(T.strangerLabel || row.word) : row.word;
        var said = row.ok ? esc(((row.scene ? row.scene + '\n' : '') + row.story).slice(0, 160))
          : '<span class="pth-err">' + esc(stoppedLead + ' ' + String(row.err || '').replace(/^Error:\s*/, '')) + '</span>';
        return '<tr class="' + (row.kind === 'stranger' ? 'pth-stranger' : '') + '">' +
          '<td class="pth-word"><code>' + esc(word) + '</code></td>' +
          '<td class="pth-story">' + said + '</td>' +
          '<td class="pth-door ' + (row.door ? 'is-ok' : 'is-none') + '">' +
          esc(row.door ? String(T.doorWord || 'door') + ' ' + row.door : String(T.noneWord || 'no door')) + '</td></tr>';
      }).join('') + '</tbody></table></div>';
  };
  PyRun.renderFloor = function (hostEl, F, results, fnName) {
    F = F || {};
    var list = results.products || [];
    var stoppedLead = String(F.stoppedLead || 'Python stopped:');
    var yourLabel = String(F.yourLabel || F.productLabel || 'Product');
    var noMachineCard = String(F.noMachineCard || '—');
    var html = '<div class="fac"><h3>' + esc(F.title || '') + '</h3><p class="fac-lead">' + esc(F.lead || '') + '</p>';
    if (!results.defined && results.ok) html += '<p class="fac-nomachine">' + esc(String(F.noMachine || '').replace('{fn}', fnName)) + '</p>';
    html += '<div class="fac-line">' + list.map(function (p) {
      var cls = p.ok ? 'is-ok' : p.none ? 'is-none' : 'is-bad';
      var shown = p.noMachine ? noMachineCard : p.err ? (stoppedLead + ' ' + p.err) : (p.none ? 'None' : p.product);
      return '<div class="fac-order ' + cls + '"><span class="fac-tag">' + esc(F.orderLabel || '') + '</span>' +
        '<span class="fac-in">' + esc(fnName + '(' + p.order.map(function (a) { return typeof a === 'number' ? a : '"' + a + '"'; }).join(', ') + ')') + '</span>' +
        '<span class="fac-tag">' + esc(F.productLabel || '') + '</span><div class="fac-product">' + esc(shown) + '</div>' +
        '<span class="fac-stamp">' + esc(p.ok ? (F.okStamp || '') : p.none ? (F.noneStamp || '') : (F.wrongStamp || '')) + '</span></div>';
    }).join('') + '</div>';
    var bad = list.filter(function (p) { return !p.ok; });
    var allNone = bad.length === list.length && bad.length > 0 && bad.every(function (p) { return p.none; });
    var pair = function (p) {
      return '<div class="fac-pair"><div><span class="fac-tag">' + esc(yourLabel) + '</span>' +
        esc(p.noMachine ? noMachineCard : p.err ? stoppedLead + ' ' + p.err : p.none ? 'None — ' + String(F.noneStamp || '') : p.product) + '</div>' +
        '<div><span class="fac-tag">' + esc(F.expectedLabel || '') + '</span>' + esc(p.expected) + '</div></div>';
    };
    html += '<div class="fac-reject"><h3>' + esc(F.rejectTitle || '') + '</h3>' + (bad.length
      ? (allNone ? '<p class="fac-reject-all">' + esc(String(F.allNoneSay || '').replace('{stamp}', String(F.noneStamp || ''))) + '</p>' + pair(bad[0])
               : '<p class="fac-reject-lead">' + esc(F.rejectLead || '') + '</p>' + bad.slice(0, 3).map(pair).join('') +
                 (bad.length > 3 ? '<p class="fac-reject-more">' + esc(String(F.moreSay || '').replace('{n}', String(bad.length - 3))) + '</p>' : ''))
      : '<p class="fac-empty">' + esc(F.rejectEmpty || '') + '</p>') + '</div></div>';
    hostEl.insertAdjacentHTML('beforeend', html);
  };
  /* THE BLOAT METER: two bars, two counts. Where the meter carries the two real
     programs (`before.lines` / `after.lines`) the counts are COMPUTED from them,
     so the number on the pupil's screen can never disagree with the program it
     describes; the authored numbers are what qa-bloat-meter holds to the same
     programs. */
  PyRun.meterHtml = function (M) {
    M = M || {};
    var before = (M.before && M.before.lines) ? M.before.lines.length : Number(M.beforeLines || 0);
    var after = (M.after && M.after.lines) ? M.after.lines.length : Number(M.afterLines || 0);
    var pct = before > 0 ? Math.max(4, Math.round(100 * after / before)) : 100;
    var unit = function (n) { return String(M.linesWord || '{n} lines').replace('{n}', String(n)); };
    return '<div class="fac-meter"><h3>' + esc(M.title || '') + '</h3>' +
      '<div class="fac-meter-row"><span>' + esc(M.beforeLabel || '') + '</span><div class="fac-meter-bar"><i style="width:100%"></i></div><b>' + esc(unit(before)) + '</b></div>' +
      '<div class="fac-meter-row"><span>' + esc(M.afterLabel || '') + '</span><div class="fac-meter-bar"><i style="width:' + pct + '%"></i></div><b>' + esc(unit(after)) + '</b></div>' +
      '<p class="fac-meter-say">' + esc(M.say || '') + '</p></div>';
  };
  /* THE STORY IS THE CONVERSATION (§C1.1 / the prototype's bench, j2-04): with a
     chunk's `chatPrints` on, every printed line is a story bubble as it is
     printed and a door line is the system line "You go through door A." (the
     chunk's own `chatLabels.doorSay`), so a room reads in order — scene,
     question, answer, road, door. One router, four call sites (worked, assembly,
     typed, staged); a chunk that does not ask keeps prints in the console only,
     exactly as every L3 card does. `needChat` is a function so a staged bench
     can create its conversation panel at the first line rather than at mount. */
  PyRun.storyRouter = function (cfg, needChat) {
    var on = !!(cfg && cfg.chatPrints);
    var buf = '';
    function line(l) {
      if (l.trim() === '') return;
      var chat = needChat();
      if (!chat) return;
      var m = PyRun.DOOR_RX.exec(l.trim());
      if (m) { chat.say('door', String((cfg.chatLabels || {}).doorSay || '').replace('{d}', m[1]), ''); return; }
      chat.say('bot', l, cfg.botWho || (cfg.chatLabels || {}).botWho || 'The bot');
    }
    return {
      on: on,
      onOut: on ? function (t) {
      ...
      flush: function () { if (on && buf.trim()) line(buf); buf = ''; }
    };
  };
  /* a feature list whose probes are the L4 kinds is judged by the L4 routines;
     everything else is PyRun.checkFeatures, untouched */
  PyRun.hasL4Probes = function (feats) {
    return (feats || []).some(function (f) {
      return f.probe === 'scene-ask' || f.probe === 'path' || f.probe === 'different' || f.probe === 'calls';
    });
  };
  PyRun.callArgsFor = function (f, b) {
    if (f.args) return f.args;
    var src = (f.argsFrom === 'orders' || !f.argsFrom) ? (f.orders || b.orders || []) : [];
    var idx = f.argIndex;
    return src.map(function (o) {
      var a = Array.isArray(o) ? o : [o];
      return idx ? idx.map(function (i) { return a[i]; }) : a;
    });
  };
  ...
  PyRun.editor = function (host, cfg) {
    ...
    /* ---- THE TWO INDENT INCREMENTS (L4 spec §C1.5, config-gated) -----------
       J2's Lesson 4 is this cohort's FIRST meeting with indentation and J3's
       Lesson 4 is its first `def` body, so the editor has to make a pushed-in
       line VISIBLE and make the next one land pushed in by itself:
         indentGuides  every line that begins with four spaces draws a faint
                       guide in the gutter, LEFT of its number and never across
                       the digits (found in the prototype's own pixels); eight
                       spaces draw two.
         autoIndent    Enter after a line ending in ':' keeps that line's indent
                       and adds four spaces; Enter on any indented line keeps
                       its indent; a plain line is left to the browser.
       Both are OFF unless the lesson asks: the L2/L3 editors name neither, so
       they render the gutter they rendered yesterday, byte for byte
       (qa-editor-indent proves it against the engine he sat). The guides are
       drawn as gutter rows only under the flag, because a `<pre>` of digits is
       what every other editor on the platform already paints. */
    var guides = !!cfg.indentGuides;
    var autoIndent = !!cfg.autoIndent;
    function paint() {
      ...
      if (guides) {
        ...
        nums.innerHTML = lines.map(function (l, i) {
          var ind = /^ {8}/.test(l) ? ' is-in2' : (/^ {4}/.test(l) ? ' is-in' : '');
          var h = rows ? Math.max(1, rows[i]) * lh : 0;
          /* THE GUIDE IS ITS OWN ELEMENT, BESIDE THE DIGIT, NEVER INSIDE ITS BOX
             (the first j2-4 / j3-4 expert walks, 14 Sep 2026): drawn as an inset
             shadow on the number's own box, the gold stripe was read by the
             readability sampler as the digit's ink — a 4.9:1 numeral measured
             at 2.1:1 — and it was not a sampler fault: the stripe really did sit
             in the glyph's box. A guide with no text of its own is measured by
             nobody, and the digit's box holds only the digit. */
          return '<div class="pye-gut"' + (h ? ' style="height:' + h + 'px"' : '') + '><span class="pye-guide' + ind + '"></span>' +
            '<span class="pye-num">' + (i + 1) + '</span></div>';
        }).join('');
      } else {
        var out = '';
        for (var i = 1; i <= n; i++) {
    ...
    ta.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' && autoIndent) {
        var a0 = ta.selectionStart;
        var before = ta.value.slice(0, a0);
        var line = before.slice(before.lastIndexOf('\n') + 1);
        var lead = (/^[ ]*/.exec(line) || [''])[0];
        var add = /:\s*$/.test(line) ? lead + '    ' : lead;
        if (add) {
          e.preventDefault();
          ta.value = before + '\n' + add + ta.value.slice(ta.selectionEnd);
          ta.selectionStart = ta.selectionEnd = a0 + 1 + add.length;
          paint();
        }
        return;
      }
  ...
  Engines.drivecheck = {
    mount: function (host, chunk, ctx) {
      ...
      /* ---- THE WAY OUT OF "COULD NOT RUN" (DFM 287, his find, 21 Sep 2026) ----
         At home the driveCheck call fails and this card used to carry ONE
         control, Try again, which ran the same call again: a pupil could press
         it for ever and never reach the OneDrive card. That is a one-way door
         with no door (the artefact check's precedent below: its skip button
         reveals once `tries` reaches two). The counter lives HERE, outside
         run(), because run() replaces the whole host on every attempt and a
         counter inside it would be born at zero each time. The way out carries
         no badge and no XP — `finishChunk(..., false)` saves a zero-XP event
         with `skipped=1` so the Live tab shows who carried on unproved, and
         the teacher can check her folders in class. Its words are content-
         owned (the 190d/192g precedent above); the fallbacks are neutral. */
      var tries = 0;
      function run() {
        host.innerHTML = '<div class="panel-loading"><span class="panel-spinner"></span><span>' +
        ...
        callWithin(ctx, 'driveCheck', { lessonNum: String(ctx.lessonEntry.num) }).then(function (r) {
          ...
          if (!r || !r.ok) {
            ...
            var errC = el('<div class="card"><h2>The inspection could not run</h2><p>' +
              ...
              '</p>' +
              (wayOut ? '<p class="dc-skip-note">' + esc(cfg.skipText || 'If the inspection still will not run, you can carry on without it and tell your teacher in class.') + '</p>' : '') +
              /* the FIRST failure renders byte-for-byte what it always did
                 (qa-clip-overlay locks that shape); only from the second does
                 the row gain its wrapper and the way out */
              (wayOut ? '<div class="rung-actions">' : '') +
              '<button class="primary-btn" type="button">Try again</button>' +
              (wayOut ? '<button class="ghost-btn dc-skip-btn" type="button">' + esc(cfg.skipLabel || 'Carry on without the inspection') + '</button></div>' : '') +
              '</div>');
            ...
            var skipBtn = errC.querySelector('.dc-skip-btn');
            if (skipBtn) skipBtn.onclick = function () { finishChunk(ctx, 'skipped=1', 0, false); };
            return;
          }
    ...
    mount: function (host, chunk, ctx) {
      ...
      function startWorked() {
        ...
        function draw() {
          ...
          host.appendChild(c);
          wireHelpRow(c, b);
          /* THE BLOAT METER (§C13, j3-04 machine-1): config-gated, drawn under the
             verdict; a worked card that names no `meter` renders as it did */
          if (cfg.meter) c.appendChild(el(PyRun.meterHtml(cfg.meter)));
          ...
          runBtn.onclick = function () {
            ...
            var storyW = PyRun.storyRouter(chat ? cfg : null, function () { return chat; });
      ...
      function startEditor() {
        ...
        /* `.pye-wall`, NOT `.pye-card` (14 Sep 2026, the first j2-4 expert walk):
           `.pye-card` is the class the SENT card <pre> wears — the platform's one
           dark monospace block — and this card wore it too, which is the exact
           collision the K41 note below describes. Nothing on the platform stood
           on this shape until j2-04's fourth extra job (a room typed from a blank
           box, with no palette, no checklist and no film — none of the wall he
           rejected), so the navy-on-navy title went unmeasured until a walker
           stood here. One class, one world (DFM 207g). */
        var isExtraW = !!cfg.extrasMode;
        var c = el('<div class="card pyrun-card pye-wall" data-build="' + esc(b.id || '') + '">' +
          '<h2 class="pyrun-goal">' + fmtBold(b.goalLine || '') + '</h2>' +
          (b.brief ? '<p class="pyrun-brief">' + fmtBold(b.brief) + '</p>' : '') +
          '<div class="pye-cols">' +
          '<div class="pye-main"><div class="pye-host"></div>' + paletteHtml + '</div>' +
          /* no jobs and no starter → no side panel: a heading over an empty list
             is a promise the card does not keep (the blank-box extra job) */
          ((feats.length || (b.starter && b.starter.length))
          ...
          '<div class="pyw-stage"></div>' +
          '<div class="pyrun-verdict" hidden></div>' +
          /* the hub's two ways out, live from mount (DFM 265c), on this shape too */
          (isExtraW ? '<div class="rung-actions pyrun-exit-row">' +
        ...
        runBtn.onclick = function () {
          ...
          /* HER RUN IS HER RUN. The conversation happens live, with her own
             ...
             diagnose is a trap). */
          var storyE = PyRun.storyRouter(chat ? cfg : null, function () { return chat; });
          ...
          liveRun.p.then(function (res) {
            ...
            if (!feats.length) {
              /* `check.kind: "clean"` on a typed build (j2-04's blank-box extra):
                 ran clean and used the lines the card names; a target-less build
                 that names no check marks as it always did */
              var kindE = String((b.check && b.check.kind) || 'target');
              var okE, usesMissingE = false;
              if (kindE === 'clean') {
                okE = res.ok;
                if (okE && b.check && b.check.uses && b.check.uses.length) {
                  usesMissingE = b.check.uses.some(function (frag) { return String(code).indexOf(frag) === -1; });
                  if (usesMissingE) okE = false;
                }
              } else okE = PyRun.matches(res.out, b.target || []);
              settle(res, okE, [], usesMissingE);
              return;
            }
        ...
        function settle(res, ok, results, usesMissing) {
          verdict.hidden = false;
          verdict.className = 'pyrun-verdict ' + (ok ? 'is-matched' : 'is-notyet');
          ...
          if (isExtraW) {
            /* a finished extra job ticks on the hub and promotes the way back — the
               assemble card's own behaviour; a Continue here ended the whole zone */
            doneJobs[b.id] = true;
            var backW = c.querySelector('.pyrun-back');
            if (backW) backW.className = 'primary-btn pyrun-back';
            return;
          }
          var last = (at + 1 >= builds.length);
          var go = el('<button class="primary-btn" type="button">' +
      ...
      function startEditorStaged() {
        ...
        var feats = b.features || [];
        var firstInsert = true;
        /* THE STAGED SHAPE INSIDE THE EXTRA JOBS (14 Sep 2026, the first j2-4
           expert walk). j2-04's fourth extra job is typed from a blank box, and
           the only editor the hub knew was the WALL he rejected (K41) — navy on
           navy, and a Continue that ended the whole zone. The staged shape is
           the ruled one, so it carries the hub's law here: the two ways out
           (back to the list, finish the lesson) live on both faces from the
           moment they mount (DFM 265c), a finished job ticks on the hub, and
           the way back is promoted to the primary action — the assemble card's
           own behaviour, mirrored. */
        var isExtraS = !!cfg.extrasMode;
        var exitRowS = isExtraS ? '<div class="rung-actions pyrun-exit-row">' +
          '<button class="ghost-btn pyrun-back" type="button">' + esc(cfg.backLabel || 'Back') + '</button>' +
          '<button class="ghost-btn pyrun-finish" type="button">' + esc(finishLabel()) + '</button>' +
          '</div>' : '';
        function wireExitS(card) {
          if (!isExtraS) return;
          App.armButton(card.querySelector('.pyrun-back'), function () { hub(); });
          App.armButton(card.querySelector('.pyrun-finish'), function () { done(); });
        }
        /* AN OPTIONAL BUILD STARTS FROM WHAT SHE ALREADY WROTE (§C13: the twist
           and the chain carry `from: "l4room"` / `"l4factory"`) — her saved draft
           is the starting text, so "your room is already in the box" is true */
        var keptCode = (b.from && ctx.draft && ctx.draft[b.from]) ? String(ctx.draft[b.from]) : '';
        /* THE TWO WORDS (§C13, j2-04 myroom): typed on the PLAN face, pinned on the
           BENCH, and the words the path check runs under */
        var wordsVals = {};
        var published = null;            /* her room number once roomPut has said so */
        var publishedCode = null;
        var state = {};                       /* feature id -> true once matched */
        var attempts = 0;
        var replies = [];
        ...
        function plan() {
          ...
          /* THE SPEC CARD (§C13, j3-04 factory): the whole table on the PLAN face */
          var S = cfg.spec;
          var specHtml = S ? '<div class="fac-spec"><h3>' + esc(S.title || '') + '</h3><p class="fac-spec-lead">' + esc(S.lead || '') + '</p>' +
          ...
          /* THE TWO WORDS BOX (§C13, j2-04 myroom) */
          var Wd = cfg.words;
          var wordsHtml = Wd ? '<div class="pth-words"><h3>' + esc(Wd.title || '') + '</h3><p class="pth-words-lead">' + esc(Wd.lead || '') + '</p>' +
            '<div class="pth-words-row">' + (Wd.boxes || []).map(function (bx) {
              return '<label>' + esc(bx.label || '') + '<input type="text" maxlength="' + Number(bx.max || 12) + '" data-w="' + esc(bx.key) + '" value="' +
                esc(wordsVals[bx.key] || '') + '" autocomplete="off" spellcheck="false"></label>';
            }).join('') + '</div><p class="pth-words-say" hidden></p></div>' : '';
          ...
          host.appendChild(c);
          wireExitS(c);
          App.armButton(c.querySelector('.pye-start'), function () {
            if (Wd) {
              /* both words, and two different ones — the refusal explains itself
                 and points at the box (DFM 205) */
              var say = c.querySelector('.pth-words-say');
              var vals = {};
              (Wd.boxes || []).forEach(function (bx) { vals[bx.key] = String(c.querySelector('[data-w="' + bx.key + '"]').value || '').trim(); });
              var keys = (Wd.boxes || []).map(function (bx) { return bx.key; });
              if (keys.some(function (k) { return !vals[k]; })) {
                say.hidden = false; say.textContent = Wd.emptySay || '';
                var first = keys.filter(function (k) { return !vals[k]; })[0];
                c.querySelector('[data-w="' + first + '"]').focus();
                return;
              }
              if (keys.length > 1 && vals[keys[0]].toLowerCase() === vals[keys[1]].toLowerCase()) {
                say.hidden = false; say.textContent = Wd.sameSay || '';
                c.querySelector('[data-w="' + keys[1] + '"]').focus();
                return;
              }
              wordsVals = vals;
            }
            bench();
          }, { repeat: true });   /* a refused press (empty or same words) must leave the button alive — App.armButton is one-shot by default, and the L4 wrong-path walk found Start dead after its own refusal (14 Sep 2026) */
        }
        ...
        function bench() {
          ...
          var Wd = cfg.words, S = cfg.spec;
          var pinnedHtml = (Wd && !b.optional)
            ? '<div class="pth-words-pinned"><span>' + esc(Wd.pinnedLabel || '') + '</span>' +
              (Wd.boxes || []).map(function (bx) { return '<code>' + esc(wordsVals[bx.key] || '') + '</code>'; }).join('') + '</div>'
            : '';
          var specHtml = S
            ? '<div class="fac-spec"><h3>' + esc(S.title || '') + '</h3><table><tbody>' + (S.rows || []).map(function (r) {
                return '<tr><td><code>' + esc(r.fn) + '(' + esc(r.slots) + ')</code></td><td>' + esc(r.gives) + '</td><td><code>' + esc(r.example) + '</code></td></tr>';
              }).join('') + '</tbody></table></div>'
            : '';
          var pubHtml = (cfg.publish && !b.optional)
            ? '<div class="pth-publish"><p class="pth-publish-note">' + esc(cfg.publish.lockedNote || '') + '</p>' +
              '<button class="ghost-btn pth-publish-btn" type="button" disabled>' + esc(cfg.publish.label || '') + '</button></div>'
            : '';
          ...
          var l4host = c.querySelector('.pyl4-host');
          var pubBtn = c.querySelector('.pth-publish-btn');
          var pubNote = c.querySelector('.pth-publish-note');
          function setPublish(ready) {
            if (!pubBtn) return;
            pubBtn.disabled = !ready;
            pubBtn.className = (ready ? 'primary-btn' : 'ghost-btn') + ' pth-publish-btn';
            if (pubNote) pubNote.hidden = !!ready;
          }
          /* editing after a publish is said, once, and the button comes back
             (§C13's `changedSay`); saying it before she has published would be a
             sentence about something that has not happened */
          ed.area.addEventListener('input', function () {
            if (published == null || publishedCode === ed.value()) return;
            var box = c.querySelector('.pth-publish');
            if (box && !box.querySelector('.pth-publish-btn')) {
              box.innerHTML = '<p class="pth-publish-note pth-changed">' + esc(cfg.publish.changedSay || '') + '</p>' +
                '<button class="ghost-btn pth-publish-btn" type="button" disabled>' + esc(cfg.publish.label || '') + '</button>';
              pubBtn = box.querySelector('.pth-publish-btn'); pubNote = box.querySelector('.pth-publish-note');
              wirePublish();
            }
          });
          function goOn() { at++; if (at < builds.length) startBuild(); else done(); }
          function wirePublish() {
            if (!pubBtn) return;
            App.armButton(pubBtn, function () {
              var P = cfg.publish;
              var code = String(ed.value());
              pubBtn.disabled = true;
              pubBtn.textContent = P.sendingSay || '';
              ctx.call('roomPut', { lessonId: ctx.lesson.id, code: code }).then(function (r) {
                if (!r || !r.ok) throw new Error((r && r.error) || 'transport');
                /* her number as the ADVENTURE will show it: the server's count
                   plus the house rooms (one derivation, roomNumberBase) */
                published = Number(r.n) + roomNumberBase(ctx);
                publishedCode = code;
                var box = c.querySelector('.pth-publish');
                box.innerHTML = '<div class="pth-published" data-arrive-live><span class="intro-kicker">' + esc(P.doneKicker || '') + '</span>' +
                  '<h3>' + esc(P.doneTitle || '') + '</h3><p>' + esc(String(P.doneSay || '').replace('{n}', String(published))) + '</p>' +
                  '<button class="primary-btn pth-published-go" type="button">' + esc(P.continueLabel || cfg.continueLabel || 'Continue') + '</button></div>';
                pubBtn = null;
                App.armButton(box.querySelector('.pth-published-go'), goOn);
              }).catch(function () {
                /* honest, and never a dead end: the button comes back, the room
                   she wrote is still in the box, and a way on that pays nothing
                   for the publish is offered beside it (DFM 265c) */
                var box = c.querySelector('.pth-publish');
                box.innerHTML = '<p class="pth-publish-note pth-fail">' + esc(P.failSay || '') + '</p>' +
                  '<button class="primary-btn pth-publish-btn" type="button">' + esc(P.label || '') + '</button>' +
                  (P.skipLabel ? '<button class="ghost-btn pth-publish-skip" type="button">' + esc(P.skipLabel) + '</button>' : '');
                pubBtn = box.querySelector('.pth-publish-btn'); pubNote = box.querySelector('.pth-publish-note');
                wirePublish();
                var skip = box.querySelector('.pth-publish-skip');
                if (skip) App.armButton(skip, goOn);
              });
            });
          }
          wirePublish();
          ...
          runBtn.onclick = function () {
            ...
            var storyS = PyRun.storyRouter(cfg, needChat);
            liveRun = PyRun.start(code, {
              limitMs: Number(cfg.limitMs || 0) || undefined,
              onOut: storyS.onOut,
            ...
            liveRun.p.then(function (res) {
              ...
              if (!feats.length) {
                /* a staged build with no jobs strip is judged like an assembly
                   card: by its target, or — `check.kind: "clean"` (the twist and
                   the chain's cousins) — by having run clean and used the lines
                   the card names; the sentence for a missing line is the card's own */
                var kindS = String((b.check && b.check.kind) || 'target');
                var okS, usesMissingS = false;
                if (kindS === 'clean') {
                  okS = res.ok;
                  if (okS && b.check && b.check.uses && b.check.uses.length) {
                    usesMissingS = b.check.uses.some(function (frag) { return String(code).indexOf(frag) === -1; });
                    if (usesMissingS) okS = false;
                  }
                } else okS = PyRun.matches(res.out, b.target || []);
                settleStaged(res, okS, [], c, verdict, ed, runBtn, usesMissingS);
                return;
              }
              /* ---- THE L4 KINDS (§C13): the path check and the floor's judge ----
                 Judged by Fable's routines from OBSERVABLE EFFECTS, in a silent
                 second pass under her declared words / the card's own orders; her
                 console shows HER run only (the L3 verdict-pass rule). */
              if (PyRun.hasL4Probes(feats)) {
                l4host.innerHTML = '';
                var pathFeats = feats.filter(function (f) { return f.probe === 'scene-ask' || f.probe === 'path' || f.probe === 'different'; });
                var callFeats = feats.filter(function (f) { return f.probe === 'calls'; });
                var results = [];
                var chain = Promise.resolve();
                if (pathFeats.length) {
                  var wk = ((cfg.words && cfg.words.boxes) || []).map(function (bx) { return wordsVals[bx.key] || ''; });
                  var wordsNow = wk.length ? wk : ((b.check && b.check.words) || []);
                  chain = chain.then(function () {
                    return PyRun.checkPaths(code, {
                      words: wordsNow,
                      stranger: (b.check && b.check.stranger) || cfg.stranger || 'banana',
                      limitMs: Number(cfg.limitMs || 0) || undefined
                    }).then(function (r) {
                      pathFeats.forEach(function (f) {
                        var ok = false;
                        if (f.probe === 'scene-ask') ok = !!r.features['scene-ask'];
                        else if (f.probe === 'different') ok = !!r.features['different'];
                        else if (f.probe === 'path') {
                          var w = String(f.word || '');
                          ok = !!(w === 'w1' ? r.features['path-1'] : w === 'w2' ? r.features['path-2'] : r.features['catch-all']);
                        }
                        results.push({ id: f.id, ok: ok });
                      });
                      if (cfg.pathTable) l4host.insertAdjacentHTML('beforeend', PyRun.pathTableHtml(cfg.pathTable, r));
                    });
                  });
                }
                callFeats.forEach(function (f) {
                  chain = chain.then(function () {
                    return PyRun.checkCalls(code, {
                      fn: f.fn, args: PyRun.callArgsFor(f, b), reference: f.reference,
                      limitMs: Number(cfg.limitMs || 0) || undefined
                    }).then(function (r) {
                      results.push({ id: f.id, ok: !!r.allOk, stopped: !r.ok });
                      if (cfg.floor && r.ok) PyRun.renderFloor(l4host, cfg.floor, r, String(f.fn));
                    });
                  });
                });
                chain.then(function () {
                  var ordered = feats.map(function (f) {
                    return results.filter(function (r) { return r.id === f.id; })[0] || { id: f.id, ok: false };
                  });
                  var stoppedAll = ordered.length > 0 && ordered.every(function (r) { return r.stopped; });
                  paintJobs(ordered, true, stoppedAll);
                  var all = ordered.every(function (r) { return r.ok; });
                  if (attempts === 1 && !b.optional) featureFirst = ordered.filter(function (r) { return r.ok; }).length;
                  settleStaged(res, all, ordered, c, verdict, ed, runBtn, false, true);
                  setPublish(all);
                });
                return;
              }
        ...
        function settleStaged(res, ok, results, c, verdict, ed, runBtn, usesMissing, l4) {
          var n = (results || []).filter(function (r) { return r.ok; }).length;
          ...
          verdict.hidden = false;
          verdict.className = 'pyrun-verdict ' + (ok ? 'is-matched' : 'is-notyet');
          /* THE VERDICT REPEATS EVERY NOT YET JOB'S NUDGE (§C13, found in the
             pixels: the strip is a screen away from the run) — for the L4 kinds,
             so a card he has already sat says exactly what it said */
          var nudges = (l4 && !ok) ? (results || []).filter(function (r) { return !r.ok; }).map(function (r) {
            var fi = -1, f = null;
            (b.features || []).forEach(function (x, i) { if (x.id === r.id) { f = x; fi = i; } });
            return (f && f.nudge) ? '<li><b>' + esc(String(cfg.jobWord || 'Job') + ' ' + (fi + 1) + ':') + '</b> ' + esc(f.nudge) + '</li>' : '';
          }).join('') : '';
          verdict.innerHTML = '<p class="pyrun-vtag">' + esc(head) + '</p>' +
            (rows ? '<ol class="pyv-list">' + rows + '</ol>' : '') +
            '<p class="pyrun-vsay">' + esc(fill(say, n)) + '</p>' +
            (nudges ? '<ol class="pyv-nudges">' + nudges + '</ol>' : '');
          if (!ok) { runBtn.disabled = false; return; }
          ...
          /* A ROOM THAT PUBLISHES IS NOT LOCKED AND CARRIES NO CONTINUE OF ITS OWN
             (§C13): the way on is the PUBLISHED card's button, and she may still
             edit and press RUN again before she publishes */
          if (cfg.publish && !b.optional) { runBtn.disabled = false; return; }
          ed.lock();
          c.querySelectorAll('.pyp-chip').forEach(function (n2) { n2.disabled = true; });
          if (cfg.extrasMode) {
            doneJobs[b.id] = true;
            var backS = c.querySelector('.pyrun-back');
            if (backS) backS.className = 'primary-btn pyrun-back';
            return;
          }
          var last = (at + 1 >= builds.length);
          var go = el('<button class="primary-btn" type="button">' +
      ...
      function startBuild(accepted) {
        ...
        /* A GAP CAN ARRIVE WITH SOMETHING ALREADY IN IT — the worked card's rule
           (see startWorked), now on a tray line too: the planted mistake sits in
           the gap, she runs it as it is, Python says so, and she fixes THAT gap
           (j3-04's swap job, DFM 197 14 Sep 2026). A gap without `pre` is empty,
           as every gap was. */
        lines.forEach(function (L) {
          (L.blanks || []).forEach(function (bl) { if (bl.pre != null && vals[bl.key] == null) vals[bl.key] = String(bl.pre); });
        });
        var attempts = 0;
        /* ---- LINES ALREADY IN PLACE (§C13, j2-04 training-3 and its extras) ----
           `fixedTop` renders LOCKED at the top of Your program — read, not moved,
           and included in the run — with `fixedTopLabel` saying so; `fixedBlanks`
           are gaps INSIDE those fixed lines (one key may fill two slots, as the
           fourth-road question does), typed like any other gap. A build that
           names neither renders byte for byte as it did. */
        var fixedTop = b.fixedTop || [];
        var fixedBlanks = b.fixedBlanks || [];
        function fixedLineHtml(t) {
          var html = esc(t);
          fixedBlanks.forEach(function (bl) {
            var slot = esc(bl.slot || '____');
            while (html.indexOf(slot) !== -1) {
              html = html.replace(slot, '<input class="pyrun-blank pyrun-fixed-blank" type="text" data-key="' + esc(bl.key) + '" size="' +
                Number(bl.size || 8) + '" maxlength="' + Number(bl.max || 40) + '" autocomplete="off" spellcheck="false" ' +
                'aria-label="' + esc(bl.label || 'type here') + '" placeholder="' + esc(bl.ph || '') + '">');
            }
          });
          return html;
        }
        var fixedHtml = fixedTop.length
          ? '<div class="pyp-fixed"><p class="pyp-fixed-label">' + esc(b.fixedTopLabel || '') + '</p><ol class="pyp-fixed-list">' +
            fixedTop.map(function (t) { return '<li class="pyp-fixed-line"><code>' + fixedLineHtml(t) + '</code></li>'; }).join('') + '</ol>' +
            (fixedBlanks.length ? '<p class="pyrun-blank-hint pyp-fixed-hint">' + esc(fixedBlanks.map(function (bl) { return bl.label; }).filter(Boolean).join(' · ')) + '</p>' : '') +
            '</div>'
          : '';
        ...
        var c = el('<div class="card pyrun-card" data-build="' + esc(b.id || '') + '">' +
          ...
          '<div class="pyrun-prog"><h3>' + esc(cfg.progLabel || 'Your program') + '</h3>' + fixedHtml + '<ol class="pyp-list"></ol></div>' +
          '</div>' +
        ...
        function codeOf() {
          var top = fixedTop.map(function (t) {
            var line = String(t);
            fixedBlanks.forEach(function (bl) {
              var slot = bl.slot || '____';
              while (line.indexOf(slot) !== -1) line = line.replace(slot, String(vals[bl.key] == null ? '' : vals[bl.key]));
            });
            return line;
          });
          return top.concat(placed.map(function (si) {
            var L = lines[si];
            var t = String(L.t || '');
            (L.blanks || []).forEach(function (bl) {
              t = t.replace(bl.slot || '____', String(vals[bl.key] == null ? '' : vals[bl.key]));
        ...
        function emptyBlank() {
          for (var f = 0; f < fixedBlanks.length; f++) {
            var fv = vals[fixedBlanks[f].key];
            if (fv == null || !String(fv).trim()) return fixedBlanks[f];
          }
        ...
        /* the gaps inside the fixed lines: one key, every slot that carries it */
        if (fixedBlanks.length) {
          c.querySelectorAll('.pyp-fixed .pyrun-blank').forEach(function (inp) {
            var k = inp.getAttribute('data-key');
            if (vals[k] != null) inp.value = vals[k];
            inp.addEventListener('input', function () {
              vals[k] = inp.value;
              c.querySelectorAll('.pyp-fixed .pyrun-blank[data-key="' + k + '"]').forEach(function (o) { if (o !== inp) o.value = inp.value; });
            });
            inp.addEventListener('keydown', function (ev) { if (ev.key === 'Enter') ev.preventDefault(); });
          });
        }
        ...
        runBtn.onclick = function () {
          ...
          attempts++;
          /* THE STORY IS THE CONVERSATION (§C1.1 / the prototype's bench, j2-04):
             with `chatPrints` on, every printed line is a story bubble as it is
             printed and a door line is the system line "You go through door A."
             — so a room reads in order: scene, question, answer, road, door. A
             chunk that does not ask keeps prints in the console only, as every
             L3 card does. */
          var storyB = PyRun.storyRouter(chatB ? cfg : null, function () { return chatB; });
          liveRun = PyRun.start(codeOf(), {
            limitMs: Number(cfg.limitMs || 0) || undefined,
            onOut: storyB.onOut,
          ...
          liveRun.p.then(function (res) {
            ...
            var ok;
            /* ---- THE L4 KINDS (§C13): judged by Fable's routines in a silent
               second pass; her live run above is HER run. `paths` reads the
               declared words, a `{key}` being a gap she filled; `calls` drops the
               card's own orders into the machine she named. The table / the floor
               is drawn under the console, where the run that produced it is. */
            if (kindB === 'paths' || kindB === 'calls') {
              var l4h = c.querySelector('.pyl4-host');
              l4h.innerHTML = '';
              var judge = kindB === 'paths'
                ? PyRun.checkPaths(codeOf(), {
                    words: ((b.check && b.check.words) || []).map(function (w) {
                      var m = /^\{(.+)\}$/.exec(String(w || ''));
                      return m ? String(vals[m[1]] == null ? '' : vals[m[1]]).trim() : String(w || '');
                    }),
                    stranger: (b.check && b.check.stranger) || cfg.stranger || 'banana',
                    limitMs: Number(cfg.limitMs || 0) || undefined
                  }).then(function (r) {
                    if (cfg.pathTable) l4h.insertAdjacentHTML('beforeend', PyRun.pathTableHtml(cfg.pathTable, r));
                    return res.ok && !!r.features['paths-all'];
                  })
                : PyRun.checkCalls(codeOf(), {
                    fn: b.check.fn, args: b.check.args || [], reference: b.check.reference,
                    limitMs: Number(cfg.limitMs || 0) || undefined
                  }).then(function (r) {
                    if (cfg.floor && r.ok) PyRun.renderFloor(l4h, cfg.floor, r, String(b.check.fn));
                    return res.ok && !!r.allOk;
                  });
  ...
  Engines.chatswap = {
    mount: function (host, chunk, ctx) {
      ...
      function reportForm(card, builderMi, solo) {
        ...
        }, { repeat: true });   /* the short-report refusal and the send failure both hand the button back; App.armButton is one-shot by default and left it dead — found by the L4 round's wrong-path audit of every refusing button (14 Sep 2026) */
      }
  ...
  /* ================= relay — "The class adventure" (j2-04, spec §C1.1) ======
     Five rooms, one after another, chosen from the rooms her class has
     published plus the three HOUSE ROOMS the content ships — so one pupil
     alone plays a whole adventure (the solo route by birth: cover day, catch-up,
     the first to finish). One room's program runs in Skulpt in chat mode: every
     print is a story bubble, `input()` opens the reply row, and the room's DOOR
     LINE (`NEXT: door A` / `B`) is never printed as story — it renders as the
     system line "You go through door A." and the engine resolves the door to a
     random PUBLISHED room not yet visited in this play (`PyRun.relayDoors`,
     Fable's routine, proved). Numbers only, never names (the Vault's secrecy
     convention). A play-through of five rooms (or every room there is, if
     fewer) earns the badge ONCE; leaving mid-play asks first and pays nothing;
     the way out is on every screen (DFM 265c).
     THE STORY MOVES FORWARD: every room starts a fresh transcript, so its scene
     is the first thing she reads (§C13, found in the pixels).
     DETERMINISTIC UNDER WALK: a harness may set `window.__relaySeed`, and the
     door rng is then Fable's seeded mulberry32 so the walkers' paths are fixed
     (DFM 199); a pupil's play is unseeded. Nothing in content can seed it.
     ROOM NUMBERS: the house rooms carry their own numbers in content (1–3); a
     published room is shown as its server number PLUS the count of house rooms,
     which is the same number the myroom card's PUBLISHED card shows — one
     derivation (`roomNumberBase`), read from the lesson's own relay chunk. */
  function roomNumberBase(ctx) {
    var chunks = (ctx && ctx.lesson && ctx.lesson.chunks) || [];
    for (var i = 0; i < chunks.length; i++) {
      if (String(chunks[i].engine) === 'relay') return ((chunks[i].config || {}).houseRooms || []).length;
    }
    return 0;
  }
  Engines.relay = {
    mount: function (host, chunk, ctx) {
      var cfg = chunk.config;
      var W = cfg.words || {};
      var L = cfg.chatLabels || {};
      var limit = Number(cfg.rooms || 5);
      var base = roomNumberBase(ctx);
      var house = (cfg.houseRooms || []).map(function (r) {
        return { n: Number(r.n), house: true, code: (r.lines || []).map(function (l) { return String(l.t || ''); }).join('\n') };
      });
      var pool = house.slice();          /* every room the play can land in */
      var codeCache = {};                /* server n -> code */
      var best = 0;                      /* the most rooms any play reached */
      var earned = false;
      var liveRun = null;
      var listed = false;
      var fill = function (t, m) {
        var out = String(t == null ? '' : t);
        Object.keys(m || {}).forEach(function (k) { out = out.split('{' + k + '}').join(String(m[k])); });
        return out;
      };
      function headFor(room) {
        return fill(room.mine ? W.roomMine : room.house ? W.roomHouse : W.roomHead, { n: room.n });
      }
      function finish(ok) {
        if (liveRun) { try { liveRun.abandon(); } catch (e) { /* settled */ } liveRun = null; }
        finishChunk(ctx, (chunk.id || 'classadventure') + '=' + best + '/' + limit, 0, ok);
      }
      /* the room list: once at open and on every Play again (§C1.1) */
      function listRooms() {
        return ctx.call('roomList', { lessonId: ctx.lesson.id }).then(function (r) {
          if (!r || !r.ok) return;
          var mine = r.mine == null ? null : Number(r.mine);
          pool = house.slice().concat((r.rooms || []).map(function (x) {
            return { n: base + Number(x.n), sn: Number(x.n), mine: mine != null && Number(x.n) === mine };
          }));
          listed = true;
        }, function () { /* an unreachable server: the house rooms are the pool, and the count says so */ });
      }
      function codeFor(room) {
        if (room.house) return Promise.resolve(room.code);
        if (codeCache[room.sn] != null) return Promise.resolve(codeCache[room.sn]);
        return ctx.call('roomGet', { lessonId: ctx.lesson.id, n: room.sn }).then(function (r) {
          if (!r || !r.ok) return null;
          codeCache[room.sn] = String(r.code || '');
          return codeCache[room.sn];
        }, function () { return null; });
      }

      /* ---- the intro, with the live count ---- */
      function intro() {
        host.innerHTML = '';
        introCard(host, {
          kicker: cfg.kicker, title: cfg.title, text: cfg.intro || '',
          steps: cfg.steps, stepsClass: 'rly-intro-steps',
          extra: '<p class="rly-count-line">' + esc(fill(W.roomsOpen, { n: pool.length })) + '</p>'
        }, cfg.beginLabel || 'Start the adventure', play);
        var line = host.querySelector('.rly-count-line');
        listRooms().then(function () { if (line) line.textContent = fill(W.roomsOpen, { n: pool.length }); });
      }

      /* ---- one play: rooms until the limit or the pool runs dry ---- */
      function play() {
        var rng = (typeof window !== 'undefined' && window.__relaySeed != null)
          ? PyRun.relayDoors.seeded(Number(window.__relaySeed)) : Math.random;
        var playState = { visited: [], limit: limit };
        var stopped = false;
        host.innerHTML = '';
        var card = el('<div class="card pyrun-card rly-card">' +
          '<div class="rly-strip"><span class="rly-room"></span><span class="rly-count"></span>' +
          '<span class="rly-open">' + esc(fill(W.roomsOpen, { n: pool.length })) + '</span></div>' +
          '<div class="pyw-stage"></div>' +
          '<div class="rung-actions rly-exit-row"><button class="ghost-btn rly-finish" type="button">' + esc(W.finishLabel || cfg.finishLabel || '') + '</button></div>' +
          '<div class="rly-leave-ask" hidden></div></div>');
        host.appendChild(card);
        var stage = card.querySelector('.pyw-stage');
        var chat = PyRun.chat(stage, L);
        /* THE WAY OUT, on the play screen itself: it asks first, says how far she
           has got, and a play she did not finish pays nothing (the duel/chatswap
           leave pattern) */
        App.armButton(card.querySelector('.rly-finish'), function () {
          if (earned) { stopped = true; finish(true); return; }
          var ask = card.querySelector('.rly-leave-ask');
          ask.hidden = false;
          ask.innerHTML = '<b>' + esc(W.leaveTitle || '') + '</b><p>' +
            esc(fill(W.leaveAsk, { done: playState.visited.length, all: limit })) + '</p>' +
            '<div class="confirm-actions"><button class="ghost-btn rly-leave-yes" type="button">' + esc(W.leaveYes || '') + '</button> ' +
            '<button class="primary-btn rly-leave-no" type="button">' + esc(W.keepGoing || '') + '</button></div>';
          App.armButton(ask.querySelector('.rly-leave-yes'), function () { stopped = true; finish(false); });
          App.armButton(ask.querySelector('.rly-leave-no'), function () { ask.hidden = true; });
        });

        function step() {
          if (stopped) return;
          var next = PyRun.relayDoors.nextRoom(playState, pool, rng);
          if (!next) { end(); return; }
          playState.visited.push(next.n);
          best = Math.max(best, playState.visited.length);
          var room = pool.filter(function (r) { return r.n === next.n; })[0];
          var head = card.querySelector('.rly-room');
          head.className = 'rly-room' + (room.mine ? ' is-mine' : '');
          head.textContent = headFor(room);
          card.querySelector('.rly-count').textContent = fill(W.roomCount, { i: playState.visited.length, all: limit });
          chat.clear();   /* a story moves forward: every room starts a fresh transcript */
          chat.say('note', '— ' + headFor(room) + ' —', '');
          codeFor(room).then(function (code) {
            if (stopped) return;
            if (code == null) {
              chat.say('note', fill(W.brokenSay, { n: room.n }), '');
              setTimeout(step, 1500);
              return;
            }
            var door = null, buf = '', wordsAfterChoice = 0, chose = false;
            function line(l) {
              var m = PyRun.DOOR_RX.exec(l.trim());
              if (m) { door = m[1]; chat.say('door', fill(W.doorSay, { d: m[1] }), ''); return; }
              if (l.trim() === '') return;
              chat.say('bot', l, L.botWho || 'The room');
              if (chose) wordsAfterChoice += l.trim().split(/\s+/).length;
            }
            var run = PyRun.runPy(code, {
              ask: function (p) { chose = true; return chat.ask(p, L.botWho, L.sendLabel); },
              onOut: function (t) {
                buf += t;
                var parts = buf.split('\n');
                buf = parts.pop();
            ...
            run.then(function (res) {
              ...
              buf = '';
              chat.closeAsk();
              if (!res.ok) chat.say('note', fill(W.brokenSay, { n: room.n }), '');
              chat.say('note', W.fetchingSay || '', '');
              /* READING TIME before the next room replaces this one: the lines
                 printed after her choice are hers to read, at the film law's pace
                 (1.2 s + 0.35 s a word) — never a half-second flash */
              var wait = Math.max(1500, 1200 + 350 * wordsAfterChoice);
              setTimeout(step, wait);
            }, function () { liveRun = null; });
          });
        }
        function end() {
          var whole = playState.visited.length >= Math.min(limit, pool.length);
          var first = !earned && whole;
          if (whole) earned = true;
          var list = playState.visited.map(String).join(', ').replace(/, (\d+)$/, ' ' + String(W.andWord || 'and') + ' $1');
          host.innerHTML = '';
          var endCard = el('<div class="card rly-end"><span class="intro-kicker">' + esc(W.endKicker || '') + '</span>' +
            '<h2>' + esc(W.endTitle || '') + '</h2>' +
            (first ? '<p class="rly-end-first">' + esc(W.endFirstSay || '') + '</p>' : '') +
            '<div class="rly-rooms">' + playState.visited.map(function (n) {
              var r = pool.filter(function (x) { return x.n === n; })[0] || {};
              return '<span class="rly-room' + (r.mine ? ' is-mine' : '') + '">' + esc(fill(W.roomHead, { n: n })) + '</span>';
            }).join('') + '</div>' +
            '<p class="intro-lead">' + esc(fill(W.endSay, { list: list })) + '</p>' +
            '<div class="confirm-actions"><button class="ghost-btn rly-again" type="button">' + esc(W.playAgainLabel || '') + '</button> ' +
            '<button class="primary-btn rly-done" type="button">' + esc(W.finishLabel || '') + '</button></div></div>');
          host.appendChild(endCard);
          App.armButton(endCard.querySelector('.rly-again'), function () { listRooms().then(play, play); });
          App.armButton(endCard.querySelector('.rly-done'), function () { finish(earned); });
        }
        step();
      }
      listRooms().then(intro, intro);
    }
  };

  /* ================= orders — "The Rush: partner orders" (j3-04, spec §C1.4) ==
     On PairKit (pair / trio / solo): gate → ORDER FORM (three orders, honesty
     floors only) → the orders cross the channel as one frame → her floor runs
     HER machines on the partner's orders (the floor judge, expectations from
     the hidden reference) → her product cards cross back by pairBlob → the
     partner CHECKS each card (a human check beside the machine's) → seal +
     reveal. Trio: A→B→C→A. Solo: the Studio's own three orders arrive. Badge 3
     pays FLAT on completing the exchange, never on the partner's verdict or the
     machine's. Leaving is on every screen and pays nothing. Records
     `rush=<phases>/4`. The side show (Unit 7) at ≥ sideAfterMs on every
     genuine wait. */
  Engines.orders = {
    mount: function (host, chunk, ctx) {
      var cfg = chunk.config;
      var W = cfg.words || {};
      var F = cfg.form || {}, U = cfg.run || {}, K = cfg.check || {}, SL = cfg.seal || {};
      var mode = null, side = null;
      var phases = 0;
      var sealed = false;
      var myOrders = null, theirOrders = null, myProducts = null, theirProducts = null;
      var myYes = null, theirYes = null;
      /* HOW MANY PRODUCTS WERE CHECKED, on each side (14 Sep 2026, qa-rush-paired):
         a factory of two machines makes SIX products from three orders, and the
         seal's sentence counted them against a literal "3" — "You said 6 of the
         3 products were right". The count travels with the verdict (CHK y/n),
         and the sentence takes it as {n}. */
      var myN = 0, theirN = 0;
      var fill = function (t, m) {
        var out = String(t == null ? '' : t);
        Object.keys(m || {}).forEach(function (k) { out = out.split('{' + k + '}').join(String(m[k])); });
      ...
      /* THE FACTORY IS HERS: the machines the factory card saved, and — where a
         pupil arrives with none — the factory's own starter shape, whose empty
         machines hand back nothing, so the products say so honestly; never the
         reference, which would make products she did not make. The floor's words
         and the two machines' references are read off the lesson's own factory
         chunk (one home, DFM 144). */
      var factory = (function () {
        var chunks = (ctx.lesson && ctx.lesson.chunks) || [];
        for (var i = 0; i < chunks.length; i++) {
          var c0 = chunks[i];
          if (String(c0.engine) !== 'pyrun') continue;
          var bs = ((c0.config || {}).builds || []);
          for (var j = 0; j < bs.length; j++) {
            if (bs[j].saveAs && (bs[j].features || []).some(function (f) { return f.probe === 'calls'; })) {
              return { cfg: c0.config, build: bs[j] };
            }
          }
        }
        return null;
      })();
      var myCode = String((ctx.draft && factory && ctx.draft[factory.build.saveAs]) ||
        (factory && (factory.build.starter || []).join('\n')) || '');
      var floorWords = (factory && factory.cfg.floor) || cfg.floor || {};
      var machines = (factory && factory.build.features || []).filter(function (f) { return f.probe === 'calls'; });

      introCard(host, {
        kicker: cfg.kicker, title: cfg.title, text: cfg.intro || '',
        steps: cfg.steps, stepsClass: 'ord-intro-steps'
      }, cfg.beginLabel || 'Open the Rush', gate);

      function stopSide(why) { if (side) { side.leave(why || 'matched'); side = null; } }
      function dropSide() { if (side) { side.stop(); side = null; } }
      function finish(earned) {
        dropSide(); PairKit.stop();
        finishChunk(ctx, (chunk.id || 'rush') + '=' + phases + '/' + Number(cfg.phaseCount || 4), 0, earned);
      }
      function leave() {
        var all = Number(cfg.phaseCount || 4);
        if (phases >= all) { finish(true); return; }
        App.confirm(cfg.leaveTitle || '', fill(cfg.leaveAsk, { done: phases, all: all }),
          cfg.leaveYes || '', function (yes) { if (yes) finish(false); });
      }
      function finishRow(extra) {
        return '<div class="rung-actions ord-exit-row">' + (extra || '') +
          '<button class="ghost-btn ord-finish" type="button">' + esc(cfg.finishLabel || '') + '</button></div>';
      }
      function wireFinish(root) {
        var b = root.querySelector('.ord-finish');
        if (b) App.armButton(b, function () { leave(); });
      }
      /* a genuine wait gets the side show (K36b), on the card that is waiting */
      function waitSide(root) {
        var slot = root.querySelector('.ord-side');
        if (!slot || side || !cfg.sideShow) return;
        setTimeout(function () {
          if (side || !slot.isConnected) return;
          side = SideShow.mount(slot, cfg.sideShow);
        }, Number(cfg.sideAfterMs == null ? 8000 : cfg.sideAfterMs));
      }
      var sign = function (mi) {
        var st = PairKit.st;
        return st ? PairKit.say('signPrefix', '') + String(st.members[mi] || '') : '';
      };
      var orderText = function (o) { return String(o[0]) + ' ' + String(o[1]); };
      var parseOrders = function (t) {
        return String(t || '').split(';').map(function (x) {
          var m = /^\s*(.+?)\s+(\d)\s*$/.exec(x);
      ...
      function onMode(m) {
        mode = m;
        if (m === 'left') { finish(false); return; }
        if (m === 'paired') { form(); return; }
        /* THE STUDIO'S ORDERS, SAID BEFORE IT HAPPENS (DFM 146e), in the words
           for the way she got here: she stopped waiting (`own`), the class has
           pairing switched off (`social`, the teacher's setting), or PairKit has
           already told her nobody is left (`solo` — its own pop has just said so,
           so the card repeats the same words rather than "you stopped waiting") */
        ownCard(m === 'own' ? 'own' : (m === 'social' ? 'off' : 'solo'));
      }
      function ownCard(why) {
        mode = 'solo';
        host.innerHTML = '';
        var title = why === 'own' ? (W.ownTitle || '') : why === 'off' ? (W.offTitle || W.soloTitle || '') : (W.soloTitle || W.ownTitle || '');
        var kicker = why === 'own' ? (W.ownKicker || '') : (W.soloKicker || W.ownKicker || '');
        var line = why === 'own' ? (W.ownLine || '') : (W.soloLine || W.ownLine || '');
        var c = el('<div class="card ord-card-own"><span class="intro-kicker">' + esc(kicker) + '</span>' +
          '<h2>' + esc(title) + '</h2><p class="intro-lead">' + fmtBold(line) + '</p>' +
          finishRow('<button class="primary-btn ord-own-go" type="button">' + esc(W.ownButton || '') + '</button>') + '</div>');
        host.appendChild(c);
        wireFinish(c);
        App.armButton(c.querySelector('.ord-own-go'), function () {
          myOrders = ((cfg.solo || {}).orders || []).slice();
          theirOrders = myOrders.slice();
          phases = Math.max(phases, 1);
          arrived();
        });
      }

      /* ---- phase 1: the order form ---- */
      function form() {
        host.innerHTML = '';
        var rows = [1, 2, 3].map(function (n) {
          return '<div class="ord-row"><span class="ord-n">' + n + '</span>' +
            '<label>' + esc(F.nameLabel || '') + '<input type="text" maxlength="12" data-name="' + n + '" autocomplete="off" spellcheck="false"></label>' +
            '<label>' + esc(F.seatsLabel || '') + '<input type="number" min="1" max="9" data-seats="' + n + '"></label></div>';
        }).join('');
        var c = el('<div class="card ord-form-card"><span class="intro-kicker">' + esc(F.kicker || '') + '</span>' +
          '<h2>' + esc(F.title || '') + '</h2><p class="intro-lead">' + fmtBold(F.lead || '') + '</p>' +
          '<div class="ord-form">' + rows + '</div><p class="ord-say" hidden></p>' +
          '<p class="ord-wait" hidden aria-live="polite"></p><div class="ord-side"></div>' +
          finishRow('<button class="primary-btn ord-send" type="button">' + esc(F.sendLabel || '') + '</button>') + '</div>');
        host.appendChild(c);
        wireFinish(c);
        var sendBtn = c.querySelector('.ord-send'), say = c.querySelector('.ord-say'), wait = c.querySelector('.ord-wait');
        App.armButton(sendBtn, function () {
          var orders = [1, 2, 3].map(function (n) {
            return [String(c.querySelector('[data-name="' + n + '"]').value || '').replace(/[;\n]/g, ' ').trim().slice(0, 12),
                    Number(c.querySelector('[data-seats="' + n + '"]').value)];
          });
          /* honesty floors only (DFM 193a): a name, and a number from 1 to 9 */
          if (orders.some(function (o) { return !o[0] || !(o[1] >= 1 && o[1] <= 9) || o[1] !== Math.floor(o[1]); })) {
            say.hidden = false; say.textContent = F.emptySay || '';
            return;
          }
          say.hidden = true;
          sendBtn.disabled = true;
          wait.hidden = false; wait.textContent = F.sendingSay || '';
          PairKit.relay(ctx, 'msg', 'ORD ' + orders.map(orderText).join('; ')).then(function (r) {
            if (!r || !r.ok) throw new Error('send');
            myOrders = orders;
            phases = Math.max(phases, 1);
            wait.textContent = F.sentSay || '';
            waitSide(c);
            if (theirOrders) arrived();
          }).catch(function () {
            sendBtn.disabled = false;
            wait.hidden = false; wait.textContent = F.failSay || '';
          });
        }, { repeat: true });   /* the refusal (a blank name, seats outside 1–9) and the send failure both hand the button back — one-shot would leave it dead (14 Sep 2026) */
        /* the partner's orders may already be in the transcript, or arrive later */
        listen();
      }
      /* THE CHANNEL, READ BY WHO SENT IT (V62/B2's law): my own frames are skipped
         first; orders come from the member who orders from me (round-robin:
         prev), products from the one who fills for me (next). */
      function listen() {
        PairKit.onEvent(function (e) {
          if (!PairKit.st || String(e[2]) !== 'msg') return;
          var from = Number(e[1]);
          if (from === Number(PairKit.st.mi)) return;
          var t = String(e[3] || '');
          if (t.indexOf('ORD ') === 0 && from === PairKit.prevRound() && !theirOrders) {
            theirOrders = parseOrders(t.slice(4));
            if (myOrders) arrived();
          } else if (t.indexOf('PROD') === 0 && from === PairKit.nextRound() && !theirProducts) {
            PairKit.blob(ctx, 'get', 'products', PairKit.nextRound()).then(function (r) {
              if (r && r.ok && Number(r.has)) { try { theirProducts = JSON.parse(String(r.v)); } catch (x) { theirProducts = null; } }
              if (theirProducts && myProducts) check();
            });
          } else if (t.indexOf('CHK ') === 0 && from === PairKit.prevRound() && theirYes == null) {
            theirYes = Number(t.slice(4).split('/')[0]) || 0;
            theirN = Number(t.slice(4).split('/')[1]) || 0;
            if (myYes != null) seal();
          }
        });
      }

      /* ---- phase 2: their orders through MY factory ---- */
      function arrived() {
        if (!theirOrders) return;
        host.innerHTML = '';
        var who = mode === 'paired' ? sign(PairKit.prevRound()) : String(W.studioName || '');
        var c = el('<div class="card pyrun-card pye-bench ord-run-card"><span class="intro-kicker">' + esc(U.kicker || '') + '</span>' +
          '<h2>' + esc(fill(U.title, { sign: who })) + '</h2><p class="intro-lead">' + fmtBold(U.lead || '') + '</p>' +
          '<div class="fac"><h3>' + esc(U.kicker || '') + '</h3><div class="fac-line">' + theirOrders.map(function (o) {
            return '<div class="fac-order"><span class="fac-tag">' + esc(floorWords.orderLabel || '') + '</span><span class="fac-in">' + esc(o[0] + ' · ' + o[1]) + '</span></div>';
          }).join('') + '</div></div>' +
          '<button class="primary-btn pyrun-run" type="button">' + esc(U.runLabel || '') + '</button>' +
          '<div class="pyw-stage"></div><div class="fac-host"></div>' +
          '<p class="ord-wait" hidden aria-live="polite"></p><div class="ord-side"></div>' +
          finishRow() + '</div>');
        host.appendChild(c);
        wireFinish(c);
        var con = PyRun.console(c.querySelector('.pyw-stage'), (factory && factory.cfg.consoleLabels) || {});
        var runBtn = c.querySelector('.pyrun-run'), wait = c.querySelector('.ord-wait');
        App.armButton(runBtn, function () {
          runBtn.disabled = true;
          con.running();
          var results = [];
          var chain = Promise.resolve();
          machines.forEach(function (f) {
            chain = chain.then(function () {
              return PyRun.checkCalls(myCode, {
                fn: f.fn, args: PyRun.callArgsFor(f, { orders: theirOrders }), reference: f.reference
              }).then(function (r) { results.push({ fn: f.fn, r: r }); });
            });
          });
          chain.then(function () {
            var first = results[0] && results[0].r;
            con.show({ ok: !!(first && first.ok), out: first ? first.out : '', err: first ? first.err : '' },
              (factory && factory.cfg.errorWords) || {});
            var fh = c.querySelector('.fac-host'); fh.innerHTML = '';
            if (first && first.ok) results.forEach(function (x) { PyRun.renderFloor(fh, floorWords, x.r, String(x.fn)); });
            if (!first || !first.ok) {
              /* her factory stopped: said plainly, with the way back to the card
                 before this one in the sentence — and RUN comes back */
              wait.hidden = false; wait.textContent = U.brokenSay || '';
              runBtn.disabled = false;
              return;
            }
            myProducts = results.map(function (x) {
              return { fn: x.fn, products: x.r.products.map(function (p) {
                return { order: p.order, product: p.noMachine ? '' : (p.err ? '' : (p.none ? 'None' : p.product)), none: !!p.none, err: !!(p.err || p.noMachine) };
              }) };
            });
            phases = Math.max(phases, 2);
            if (mode !== 'paired') { theirProducts = myProducts; check(); return; }
            wait.hidden = false; wait.textContent = fill(U.sendingSay, { sign: sign(PairKit.prevRound()) });
            PairKit.blob(ctx, 'put', 'products', JSON.stringify(myProducts)).then(function (r) {
              if (!r || !r.ok) throw new Error('blob');
              return PairKit.relay(ctx, 'msg', 'PROD sent');
            }).then(function () {
              wait.textContent = U.sentSay || '';
              waitSide(c);
              if (theirProducts) check();
            }).catch(function () {
              wait.textContent = F.failSay || '';
              runBtn.disabled = false;
            });
          });
        }, { repeat: true });   /* a broken run (brokenSay) and a failed send both re-enable RUN, so it must stay armed (14 Sep 2026) */
      }

      /* ---- phase 3: she checks the products made from HER orders ---- */
      function check() {
        if (!theirProducts || !myOrders) return;
        dropSide();
        host.innerHTML = '';
        var who = mode === 'paired' ? sign(PairKit.nextRound()) : String(W.studioName || '');
        var lead = mode === 'paired' ? fill(K.lead, { sign: who }) : String(K.ownLead || fill(K.lead, { sign: who }));
        var cards = [];
        theirProducts.forEach(function (m) {
          (m.products || []).forEach(function (p) { cards.push({ fn: m.fn, p: p }); });
        });
        var c = el('<div class="card ord-check-card"><span class="intro-kicker">' + esc(K.kicker || '') + '</span>' +
          '<h2>' + esc(K.title || '') + '</h2><p class="intro-lead">' + fmtBold(lead) + '</p>' +
          '<div class="ord-check">' + cards.map(function (x, i) {
            var call = x.fn + '(' + (x.p.order || []).map(function (a) { return typeof a === 'number' ? a : '"' + a + '"'; }).join(', ') + ')';
            var made = x.p.none || x.p.err || !x.p.product ? (K.noneStamp || '') : x.p.product;
            return '<div class="ord-card' + (x.p.none || x.p.err ? ' is-none' : '') + '">' +
              '<div><span class="fac-tag">' + esc(K.orderLabel || '') + '</span><code>' + esc(call) + '</code></div>' +
              '<div><span class="fac-tag">' + esc(K.productLabel || '') + '</span><code class="ord-made">' + esc(made) + '</code></div>' +
              '<div class="ord-yn"><button class="ghost-btn" data-yn="y" data-i="' + i + '" type="button">' + esc(K.yes || '') + '</button>' +
              '<button class="ghost-btn" data-yn="n" data-i="' + i + '" type="button">' + esc(K.no || '') + '</button></div></div>';
          }).join('') + '</div><p class="ord-wait" hidden aria-live="polite"></p><div class="ord-side"></div>' + finishRow() + '</div>');
        host.appendChild(c);
        wireFinish(c);
        var answers = {};
        c.querySelectorAll('[data-yn]').forEach(function (b) {
          b.addEventListener('click', function () {
            answers[b.getAttribute('data-i')] = b.getAttribute('data-yn');
            c.querySelectorAll('[data-i="' + b.getAttribute('data-i') + '"]').forEach(function (x) { x.classList.toggle('is-on', x === b); });
            if (Object.keys(answers).length < cards.length || myYes != null) return;
            myYes = Object.keys(answers).filter(function (k) { return answers[k] === 'y'; }).length;
            myN = cards.length;
            phases = Math.max(phases, 3);
            var w = c.querySelector('.ord-wait');
            w.hidden = false;
            if (mode !== 'paired') { w.textContent = K.ownDoneSay || ''; setTimeout(seal, 900); return; }
            w.textContent = K.doneSay || '';
            waitSide(c);
            PairKit.relay(ctx, 'msg', 'CHK ' + myYes + '/' + cards.length).then(function () {
              if (theirYes != null) seal();
            });
          });
        });
      }
      ...
      function seal() {
        ...
        dropSide();
        phases = Math.max(phases, Number(cfg.phaseCount || 4));
        host.innerHTML = '<div class="panel-loading"><span class="panel-spinner"></span><span>' + esc(SL.sealingSay || '') + '</span></div>';
        var go = function (names) {
          host.innerHTML = '';
          var own = mode !== 'paired';
          var prev = own ? -1 : PairKit.prevRound();
          var name = (!own && names && names[prev]) ? String(names[prev]) : '';
          var c = el('<div class="card ord-seal"><span class="intro-kicker">' + esc(SL.sealKicker || '') + '</span>' +
            '<h2>' + esc(own ? (SL.ownSealTitle || '') : (SL.sealTitle || '')) + '</h2>' +
            '<p class="intro-lead">' + fmtBold(own ? fill(SL.ownSealSay, { yes: myYes == null ? 0 : myYes, n: myN })
              : fill(SL.sealSay, { yes: theirYes == null ? 0 : theirYes, n: theirN || myN, sign: sign(prev), name: name })) + '</p>' +
            '<button class="primary-btn ord-done" type="button">' + esc(cfg.afterLabel || '') + '</button></div>');
          host.appendChild(c);
          App.armButton(c.querySelector('.ord-done'), function () { finish(true); });
        };
        if (mode === 'paired' && PairKit.st) {
          PairKit.complete(ctx).then(function (r) { go((r && r.names) || null); }, function () { go(null); });
        } else go(null);
      }
    }
  };
  ...
  Engines.duel = {
    mount: function (host, chunk, ctx) {
      ...
      function drawRound() {
        ...
        }, { repeat: true });   /* the empty refusal must leave the lock alive; found by the L4 round's wrong-path walker on the same one-shot default (14 Sep 2026) */
