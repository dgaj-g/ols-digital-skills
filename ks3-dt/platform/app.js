...
(function (global) {
  ...
  App.fetchContent = function (path) {
    ...
    /* THE VERSION GOES IN THE URL, and this is the half of DFM 189 that was
       never closed. That rule fixed the localStorage key: a new contentVersion
       means a new key, so a stale lesson can no longer be served out of it.
       Underneath it, GitHub Pages sends `cache-control: max-age=600`, so for ten
       minutes the BROWSER'S OWN HTTP cache answers this fetch without asking
       Pages at all — and the stale body it returns was then written into
       localStorage under the NEW key. The old content got laundered into the new
       version's slot and stayed there for good, because purgeOldContent only
       removes entries whose version DIFFERS.
       That is exactly what he hit on 17 Aug: Pages was serving the new lesson,
       the server was reporting the new version, a private window (empty HTTP
       cache) showed the new wording, and his ordinary tab kept the old one
       through repeated hard refreshes.
       Putting the version in the query string makes the URL change when the
       content changes, so a new version can never be answered from a cache of
       the old one — while a repeat load of the SAME version still comes from
       cache, which is what keeps a C2k machine quick. */
    var url = CONTENT_BASE + path + (App.state.contentVersion ? '?v=' + encodeURIComponent(App.state.contentVersion) : '');
  ...
  /* ---------------- the year's own view of the kit registry ----------------
     HIS K1 RULING, 14 Aug 2026: J2 and J3 must not inherit J1's agent fiction,
     its rank names or its wardrobe. One registry file still holds everything —
     adding a look is still a git push, no redeploy — but each year reads only
     its own slice of it: its rank ladder, the looks tagged for it, and its own
     kit NAME and explainer.
     Resolved ONCE here, at load, so every reader downstream (the clearance pop,
     the kit modal, the theme painter) is untouched and J1 renders exactly as it
     did. The server does the identical filtering in apiSetKit, so this is a
     convenience for her screen and never the security boundary.
     THE LEGACY READS ARE DELIBERATE (rule 42/35 family — never strand a pupil):
     a browser holding a themes.json cached from before this change has no
     clearancesByYear and no year tags, and localStorage survives a reload
     (DFM 189), so the old `clearances` key is still honoured and a look that
     names no year is treated as J1's. And if filtering ever leaves her with
     nothing to wear, midnight is put back — an empty wardrobe is a broken
     screen, not a rule. */
  App.resolveKit = function (reg, year) {
    if (!reg) return reg;
    var y = String(year || 'j1');
    var out = {}, k;
    for (k in reg) if (Object.prototype.hasOwnProperty.call(reg, k)) out[k] = reg[k];
    var by = reg.clearancesByYear;
    out.clearances = (by && by[y]) || (by && by.j1) || reg.clearances || [];
    var mine = function (item) {
      var t = item && item.year;
      return t == null ? (y === 'j1') : (t === 'all' || String(t) === y);
    };
    out.themes = (reg.themes || []).filter(mine);
    out.insignia = (reg.insignia || []).filter(mine);
    ...
    return App.call('state').then(function (r) {
      ...
      return App.fetchContent(s.year + '/manifest.json').then(function (man) {
        ...
        return App.fetchContent('themes.json').then(
          function (reg) { s.kit = App.resolveKit(reg, s.year); },
          function () { s.kit = null; }
        );
      }).then(function () {
        App.applyKit();
  ...
  function maybeClearancePop() {
    ...
    ov.innerHTML = '<div class="badge-pop-card">' +
      ...
      /* DAMIEN, 31 Jul 2026: a pupil has never met the term "Agent Kit", so the
         ...
         14 Aug 2026 (his K1 ruling): the NAME and the explaining sentence now come
         from the registry, because J2's is a toolbench and J3's is a studio and
         neither is an Agent Kit. J1's registry values reproduce this wording
