...
The first token is the **inbox issue number** (e.g. `11`). If no number was given, run `gh issue list --repo dgaj-g/ols-digital-skills-inbox --state open --limit 20` AND `gh issue list --repo dgaj-g/ols-digital-skills-inbox --state closed --limit 10`, list both sets, and ask Damien which to publish.
