...
0. **Orient** — read the README, the shared `style.css`, `intro-loader.js`, and **skim the reference gallery** (playbook → "The reference gallery"), opening the build whose *shape* is closest to the brief. `chemistry/mendeleev-cards/` is the house-feel reference, **not** the only bar. A fresh session needs this context.
...
6. **Choose the activity pattern** (drag-drop, flashcards, timeline, quiz, interactive diagram, story-map, login-gated board, project builder, or your judgement) — and check the **capability toolbox** (playbook → Step 6b) for a free library that would lift the build.
...
**Quality bar:** the bar is the **whole reference gallery** (playbook), not one app — Mendeleev (drag-drop polish), Costa Rica (story-map), Constitution (buildable + collaborative), Mon Carnet (login-gated project → real Google Doc), Girls Coding (server-scored competition + leaderboard + teacher admin), Glass Jotter (working capture + marking engine + markbook). Match the shape closest to the brief, then aim to *add* to the gallery.
