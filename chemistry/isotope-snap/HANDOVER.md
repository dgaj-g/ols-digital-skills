...
- **Path B server** = **execute-as-user + ScriptProperties** model (Mon Carnet style), NOT the Sheet/execute-as-me model. WHY: this lets the server **auto-read the pupil's verified C2k name** (proven 14 Jun 2026 — pupils return full first name via the OIDC userinfo endpoint, staff return an initial). `autoName_()` fetches `https://openidconnect.googleapis.com/v1/userinfo` with `ScriptApp.getOAuthToken()`; `apiWhoAmI` returns the name so the front-end **skips the name form entirely** (zero-typing sign-in; email-prefill type-once kept as fallback). Storage keys: `staffPasscode`, `classes`, `p:<class>:<email>` (pupil record), `groups:<class>`, `reveal:<class>`. Deploy: **Execute as: User accessing**, within-domain; manifest `server/appsscript.json` declares `userinfo.profile`+`userinfo.email`+`external_request`+`scriptapp` scopes. Client API contract unchanged. `OLS_BOOT` injection in `doGet` preserved. ASCII assembler unchanged. Gotchas: `LockService` on writes, primitive coercion, passcode trim/lowercase. One-time per-pupil Google consent (admin can pre-trust). See [[reference_c2k_name_extraction]] in memory.
...
- ✅ Draft PR dgaj-g/ols-digital-skills#24, inbox comment on #19, PushNotification sent. Playbook updated (3D + groups patterns, .slot gotcha). **BUILD COMPLETE — awaiting Damien's review. Next step is /publish #19 (login-gated variant) once reviewed + deployed with Teresa.**
